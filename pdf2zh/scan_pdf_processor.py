...
class ScanPDFProcessor:
    ...
    def analyze_layout(self, page_image):
        ...
        for col_x0, col_x1 in columns:
            ...
            regions.append(LayoutRegion(
                bbox=(col_x0/self.scale, text_bot/self.scale,
                      col_x1/self.scale, text_top/self.scale),
                region_type='text'
            ))
        header_h = h * 0.08
        footer_h = h * 0.08
    ...
    def pdf_to_pixel_coords(self, bbox, img_h):
        x0, y0, x1, y1 = bbox
        return (int(x0 * self.scale), int(img_h - y1 * self.scale),
                int(x1 * self.scale), int(img_h - y0 * self.scale))
    ...
    def _sort_by_reading_order(self, segments):
        ...
        for i, seg in enumerate(segments):
            graph.add_node(TextNode(id=i, x0=seg.bbox[0], y0=seg.bbox[1], x1=seg.bbox[2], y1=seg.bbox[3], text=seg.text, page_num=seg.page_num))
        sorted_nodes = graph._spatial_sort()
        id_map = {n.id: i for i, n in enumerate(sorted_nodes)}
        seg_list = list(segments)
        return sorted(seg_list, key=lambda s: id_map.get(seg_list.index(s), float('inf')))
