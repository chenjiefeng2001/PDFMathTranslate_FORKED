    ...
    with QUEUE_LOCK:
        for pos, cid in enumerate(TASK_QUEUE):
            if cid in GLOBAL_TASK_STORE:
                ...
                GLOBAL_TASK_STORE[cid][
                    "label"
                ] = f"⏳ 排队中（位置 {pos + 1}/{len(TASK_QUEUE)}）"
...
def download_with_limit(url, save_path, size_limit):
    ...
    with requests.get(url, stream=True, timeout=10) as resp:
        ...
        try:
            msg = Message()
            msg["Content-Disposition"] = content
...
def _check_pdf_has_text(fp, sample_pages=10):
    try:
        ...
        if tlen == 0:
            return (
                False,
                f"该PDF共{total}页，检测前{sample_pages}页未发现可提取的文字内容。\n\n📌 可能原因：扫描版/加密文件\n💡 建议方案：使用OCR工具",
            )
        if tlen < 10:
            return True, f"⚠️ 仅检测到少量文本（{tlen}字符）"
        return True, ""
    except Exception:
        logger.warning("PDF文本预检测失败", exc_info=True)
        return True, ""


def on_upload_files(files, fl_state):
    if not files:
        return fl_state, "", gr.update(), gr.update()
    ...
    return (
        existing,
        f"已上传 {len(existing)} 个文件",
        gr.update(value=html),
        gr.update(value=existing[0]["path"] if existing else None),
    )
...
def on_file_input_change(files, fl_state):
    ...
    for e in existing:
        if e["path"] in cur:
            ...
            removed += 1
            logger.info(f"同步移除: {e.get('name','unknown')}")
    if removed > 0:
        gr.Info(f"已移除 {removed} 个文件", duration=2)
...
def _render_file_list(fl):
    ...
    for f in fl:
        ...
        msg = f.get("message", "")
        c = sc.get(st, "#888")
        ic = si.get(st, "\U0001f4c4")
        m = (
            f'<span style="color:{c};font-size:12px;margin-left:8px">{msg}</span>'
            if msg
            else ""
        )
        items.append(
            f'<div style="display:flex;align-items:center;padding:4px 8px;border-bottom:1px solid var(--border-color-primary);gap:8px;"><span>{ic}</span><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px;" title="{name}">{name}</span><span style="width:8px;height:8px;border-radius:50%;background:{c};"></span><span style="color:{c};font-size:12px;">{st}</span>{m}</div>'
        )
...
def _prog(pct, label=""):
    p = max(0, min(100, pct))
    l = (
        f'<div style="font-size:13px;color:var(--body-text-color);margin-bottom:4px;font-weight:500;">{label}</div>'
        if label
        else ""
    )
    return (
...
def submit_translation_task(
    ...
    if flag_demo and not verify_recaptcha_response(recaptcha_response):
    ...
    if client_id in GLOBAL_TASK_STORE:
        ...
        if current_status in ["pending", "queued", "translating"]:
            raise gr.Error(
                "您当前已有任务正在执行或排队中，请等待完成或手动停止后再提交新任务！"
            )
    ...
    if file_type == "File":
        if fl_state and len(fl_state) > 0:
            ...
            files_to_process = (
                [file_input] if isinstance(file_input, str) else list(file_input)
            )
        ...
        fp = download_with_limit(
            link_input, output, 5 * 1024 * 1024 if flag_demo else None
        )
    ...
    if fl_state:
        for fe in fl_state:
            ...
            copied_fl_state.append(
                {**fe, "status": st, "message": "缓存命中" if st == "cached" else ""}
            )
    ...
    for fp, v in cached.items():
        if isinstance(v, dict):
            all_results[fp] = (str(v.get("mono_path", "")), str(v.get("dual_path", "")))
        else:
            all_results[fp] = (
                (str(v[0]), str(v[1])) if isinstance(v, (tuple, list)) else ("", "")
            )
...
def background_translation_worker(args):
    ...
    try:
        for idx, fp in enumerate(files_to_process):
            ...
            if skip_event.is_set():
                ...
                if fl_state:
                    for i, fe in enumerate(fl_state):
                        if fe.get("path") == fp:
                            fl_state[i]["status"] = "skipped"
                            fl_state[i]["message"] = "用户跳过"
                            break
                skipped += 1
                store["skipped"] = skipped
                store["file_list"] = fl_state
                store["file_list_html"] = _render_file_list(fl_state)
                continue
            fname = os.path.basename(fp)
            ...
            if len(str(output / f"{safe}-mono.pdf")) > max_p:
                safe = uuid.uuid4().hex[:16]
            # 使用 client_id 子文件夹隔离每个任务的文件，防止多用户文件覆盖
            client_output_dir = output / str(client_id)
            client_output_dir.mkdir(parents=True, exist_ok=True)
            raw = client_output_dir / f"{safe}.pdf"
            mono = client_output_dir / f"{safe}-mono.pdf"
            dual = client_output_dir / f"{safe}-dual.pdf"
            try:
                shutil.copy2(fp, raw)
                ...
                if fl_state:
                    for i, fe in enumerate(fl_state):
                        if fe.get("path") == fp:
                            fl_state[i]["status"] = "error"
                            fl_state[i]["message"] = f"复制失败: {str(e)[:40]}"
                            break
                errors += 1
                store["errors"] = errors
                store["file_list"] = fl_state
            ...
            try:
                ...
                logger.error(f"PDF检测失败 {fname}: {e}")
                errors += 1
                store["errors"] = errors
                if raw.exists():
                    raw.unlink()
            ...
            try:
                ...
                while True:
                    while pause_event.is_set() and not cancellation_event.is_set():
                        time.sleep(0.5)
                    if cancellation_event.is_set():
                        break
                    try:
                        ...
                        if msg_type == "DONE":
                            ...
                            store["current_label_raw"] = last_lbl
                            scp = (
                                (completed + errors + (last_val / 100.0))
                                / max(total_files, 1)
                                * 100
                            )
                            store["total_progress"] = scp
                            store["label"] = f"({idx+1}/{total_to_do}) {last_lbl}"
                            store["progress"] = scp
                    except queue.Empty:
                        pass
                # 清理线程安全拦截器的注册
                ...
                if cancellation_event.is_set():
                    raise CancelledError("用户已手动停止任务")
                ...
                try:
                    ...
                    logger.warning(f"输出验证失败 {fname}: {ve}")
                try:
                    fh = compute_file_hash(str(raw))
                    set_file_cache(
                        file_hash=fh,
                        file_name=fname,
                        lang_in=lang_in,
                        ...
                        mono_path=str(mono),
                        dual_path=str(dual),
                        page_range=str(pages) if pages else "",
                    )
                except Exception as ce:
                    logger.debug(f"缓存记录失败: {ce}")
                all_results[fp] = (str(mono), str(dual))
                completed += 1
                store["completed"] = completed
                store["all_results"] = all_results
                if fl_state:
                    for i, fe in enumerate(fl_state):
                        if fe.get("path") == fp:
                            fl_state[i]["status"] = "done"
                            fl_state[i]["message"] = "翻译完成"
                            break
                store["file_list"] = fl_state
                store["file_list_html"] = _render_file_list(fl_state)
                store["file_progress"] = 100
                ...
                if fl_state:
                    for i, fe in enumerate(fl_state):
                        if fe.get("path") == fp:
                            fl_state[i]["status"] = "error"
                            fl_state[i]["message"] = f"错误: {str(e)[:50]}"
                            break
                errors += 1
                store["errors"] = errors
                for t in [raw, mono, dual]:
        ...
        if client_id in GLOBAL_TASK_STORE:
            store = GLOBAL_TASK_STORE[client_id]
            success = [
                fp for fp, v in all_results.items() if v[0] and os.path.exists(v[0])
            ]
            ...
            if cancellation_event.is_set():
                ...
                store["label"] = f"⏹ 用户已手动停止任务" + (
                    f" (已生成 {len(result_files)} 个文件)" if result_files else ""
                )
            elif result_files:
                if errors > 0 or skipped > 0:
                    store["status"] = "done"
                    store["label"] = (
                        f"⚠️ 任务结束 (成功 {len(success)} 个，失败 {errors} 个，跳过 {skipped} 个)"
                    )
                else:
                    store["status"] = "done"
                    store["label"] = "✅ 全部翻译完成"
                store["file_progress"] = 100
                store["total_progress"] = 100
            else:
                store["status"] = "error"
        ...
        if client_id in GLOBAL_TASK_STORE:
            GLOBAL_TASK_STORE[client_id]["status"] = "error"
            GLOBAL_TASK_STORE[client_id]["label"] = f"❌ 后台错误: {str(e)[:80]}"
    finally:
        cancellation_event_map.pop(client_id, None)
        pause_event_map.pop(client_id, None)
        skip_event_map.pop(client_id, None)
...
def sync_status_from_backend(client_id):
    ...
    show_file_selector = (
        gr.update(visible=has_results, choices=file_choices, value=current_val)
        if has_results
    ...
    show_zip = (
        gr.update(
            visible=has_results,
            value=result_zip if result_zip and os.path.exists(result_zip) else None,
        )
        if has_results
    ...
    show_single_file = (
        gr.update(
            visible=has_results,
            value=(
                preview_path if preview_path and os.path.exists(preview_path) else None
            ),
        )
        if has_results
        else gr.update(visible=False, value=None)
    ...
    return (
        ...
        show_zip,
        show_preview,
        gr.update(value=fl_html),
        gr.update(value=file_icon),
...
def pause_translate_task(client_id):
    if client_id and client_id in pause_event_map:
        pause_event_map[client_id].set()


def resume_translate_task(client_id):
    if client_id and client_id in pause_event_map:
        pause_event_map[client_id].clear()


def skip_current_task(client_id):
    if client_id and client_id in skip_event_map:
        skip_event_map[client_id].set()
...
head_content = (
    frontend_fixes + (demo_recaptcha if flag_demo else "") + session_recovery_js
)
...
with gr.Blocks(
    title="PDFMathTranslate - PDF Translation",
    theme=gr.themes.Soft(primary_hue=custom_blue, spacing_size="md", radius_size="lg"),
    css=custom_css,
    head=head_content,
) as demo:
    gr.Markdown(
    ...
    with gr.Row(equal_height=False):
        with gr.Column(scale=4, min_width=350):
            ...
            file_type = gr.Radio(
                choices=["File", "Link"], label="Type", value="File", show_label=False
            )
            file_input = gr.File(
                label="上传文件（支持多选 / PDF, DOC, DOCX）",
                file_count="multiple",
            ...
            page_input = gr.Textbox(
                label="自定义页码范围（如 1-5, 8）", visible=False, interactive=True
            )
            with gr.Accordion("🛠️ 高级与实验性选项", open=False):
                ...
                with gr.Row():
                    threads = gr.Textbox(
                        label="并发线程数", interactive=True, value="4"
                    )
                    skip_subset_fonts = gr.Checkbox(
                        label="Skip font subsetting (防乱码)",
                        interactive=True,
                        value=False,
                    )
                    ignore_cache = gr.Checkbox(
                        label="忽略缓存重新翻译", interactive=True, value=False
                    )
                gr.Markdown("#### 📐 公式与排版规则")
                with gr.Row():
                    vfont = gr.Textbox(
                    ...
                    vchar = gr.Textbox(
                        label="公式字符正则 (vchar)",
                        interactive=True,
                        value=ConfigManager.get("PDF2ZH_VCHAR", ""),
                        placeholder="需要保留不翻译的特殊字符",
                    )
                prompt = gr.Textbox(
                    label="LLM 提示词 (Prompt) - 仅限大模型服务有效",
                    interactive=True,
                    visible=False,
                )
                envs.append(prompt)
            ...
            def on_select_service(sv):
                ...
                for i, e in enumerate(t.envs.items()):
                    l = e[0]
                    v = ConfigManager.get_env_by_translatername(t, e[0], e[1])
                    vis = True
                    if hidden_gradio_details:
                        if "MODEL" not in str(l).upper() and v:
                            vis = False
                        if "API_KEY" in l.upper():
                            v = "***"
                    _envs[i] = gr.update(visible=vis, label=l, value=v)
                _envs[-1] = gr.update(visible=t.CustomPrompt)
                return _envs

            def on_select_filetype(ft):
                return gr.update(visible=ft == "File"), gr.update(visible=ft == "Link")

            def on_select_page(c):
                return gr.update(visible=c == "Others")

            def on_vfont_change(v):
                ConfigManager.set("PDF2ZH_VFONT", v)
                return v
            ...
            file_type.select(
                ...
                js=(
                    f"""(a,b)=>{{try{{grecaptcha.render('recaptcha-box',{{'sitekey':'{client_key}','callback':'onVerify'}});}}catch(error){{}}return [a];}}"""
                    if flag_demo
                    else ""
                ),
            )
            ...
            client_id_state = gr.Textbox(
                elem_id="client_id_state", elem_classes=["hidden-ele"]
            )
            hidden_sync_btn = gr.Button(
                "sync", elem_id="hidden-sync-btn", elem_classes=["hidden-ele"]
            )
            ...
            with gr.Row(elem_classes="control-group"):
                ...
                cancellation_btn = gr.Button("⏹ 停止", variant="stop")
            gr.HTML(
                "<hr style='margin:12px 0;border-top:1px dashed var(--border-color-primary);' />"
            )
            file_list_state = gr.State([])
            with gr.Column(elem_classes="status-board"):
                file_list_summary = gr.Markdown(
                    value="等待上传文件...", elem_classes=["summary-text"], visible=True
                )
                file_list_html = gr.HTML(value="", visible=True)
            ...
            recaptcha_response = gr.Textbox(
                label="reCAPTCHA", elem_id="verify", visible=False
            )
            recaptcha_box = gr.HTML('<div id="recaptcha-box"></div>')
            with gr.Accordion("Technical details", open=False):
                gr.Markdown(tech_details)
        with gr.Column(scale=7):
            with gr.Column(elem_classes="pdf-wrapper"):
                preview = PDF(
                    label="Document Preview", show_label=False, visible=True, height=750
                )
    file_input.upload(
        on_upload_files,
        inputs=[file_input, file_list_state],
        outputs=[file_list_state, file_list_summary, file_list_html, preview],
        js=(
            f"""(a,b)=>{{try{{grecaptcha.render('recaptcha-box',{{'sitekey':'{client_key}','callback':'onVerify'}});}}catch(error){{}}return [a];}}"""
            if flag_demo
            else None
    ...
    demo.load(
        fn=None, inputs=None, outputs=client_id_state, js="() => window.getClientId()"
    )
...
def parse_user_passwd(fp):
    ...
    if fp and len(fp) == 2:
        try:
            content = open(fp[1], "r", encoding="utf-8").read()
        except FileNotFoundError:
            print(f"Error: File '{fp[1]}' not found.")
    if fp:
        try:
            tups = [
                tuple(l.strip().split(","))
                for l in open(fp[0], "r", encoding="utf-8")
                if l.strip()
            ]
        except FileNotFoundError:
            print(f"Error: File '{fp[0]}' not found.")
    return tups, content


def _has_ipv6():
    try:
        return socket.socket(socket.AF_INET6, socket.SOCK_STREAM).close() or True
    except OSError:
        return False
...
def setup_gui(share=False, auth_file=["", ""], server_port=7860):
    ...
    if flag_demo:
        demo.launch(server_name=_host, max_file_size="5mb", inbrowser=True)
        return
    demo.launch(
        server_name=_host,
        debug=False,
