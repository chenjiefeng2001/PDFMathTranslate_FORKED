...
DEFAULT_TEMPERATURE_MAP = {
    NodeType.FORMULA: 0.0,
    NodeType.CODE: 0.0,
    NodeType.REFERENCE: 0.0,
    NodeType.HEADING: 0.1,
    NodeType.ABSTRACT: 0.1,
    NodeType.CAPTION: 0.2,
    NodeType.PARAGRAPH: 0.3,
}
...
class ModelRouter:
    def __init__(
        self, model_map=None, temperature_map=None, default_model="gpt-4o-mini"
    ):
    ...
    def route(self, node):
        ...
        mt = (
            8192
            if node.node_type in (NodeType.PARAGRAPH, NodeType.ABSTRACT, NodeType.TABLE)
            else 2048
        )
...
class TranslationSession:
    ...
    def fail(self, error):
        self._finished_at = time.time()
        self._status = "failed"
        self._errors["_session"] = error

    def record_result(self, node_id, text):
        self._results[node_id] = text
    ...
    def summary(self):
        return {
            ...
            "nodes_translated": len(self._results),
            "errors": len(self._errors),
            "elapsed": round(self.elapsed_seconds or 0, 2),
