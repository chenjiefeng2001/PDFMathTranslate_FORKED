...
_STOPWORDS = {
    ...
    "from", "into", "over", "under", "using", "used", "uses", "use",
    "where", "which", "what", "then", "than", "when", "its", "it",
    "on", "in", "is", "of", "to", "as", "an", "by", "at", "be", "not",
}
...
class ReviewAgent:
    ...
    def __init__(self, glossary: Optional[Dict[str, str]] = None,
                 require_translation: bool = True) -> None:
        self.glossary = dict(glossary or {})
        self.require_translation = require_translation

    def check_formula_integrity(self, source: str, translated: str,
                                node_id: str) -> List[ReviewIssue]:
        issues = []
        # Only meaningful when the source actually contains math operators.
        if _MATH_OPS_RE.search(source) is None:
            return issues
        src_tokens = {t for t in _MATH_TOKEN_RE.findall(source)
                      if t.lower() not in _STOPWORDS}
        tr_tokens = {t for t in _MATH_TOKEN_RE.findall(translated)
                     if t.lower() not in _STOPWORDS}
        ...
        if missing_alpha:
            issues.append(ReviewIssue(
                code="FORMULA_CHANGED", node_id=node_id,
                message=f"Formula tokens missing: {sorted(missing_alpha)[:8]}",
                severity="error",
            ))
        return issues

    def check_numbers(self, source: str, translated: str,
                      node_id: str) -> List[ReviewIssue]:
        ...
        if missing:
            return [ReviewIssue(
                code="NUMBER_CHANGED", node_id=node_id,
                message=f"Numbers missing: {sorted(missing)[:8]}",
                severity="warning",
            )]
        return []

    def check_glossary(self, source: str, translated: str,
                       node_id: str) -> List[ReviewIssue]:
        issues = []
        for term, pinned in self.glossary.items():
            if term.lower() in source.lower() and pinned not in translated:
                issues.append(ReviewIssue(
                    code="GLOSSARY_VIOLATION", node_id=node_id,
                    message=f"'{term}' should appear as '{pinned}'",
                    severity="warning",
                ))
        return issues

    def check_identity(self, source: str, translated: str,
                       node_id: str) -> List[ReviewIssue]:
        if self.require_translation and source.strip() and \
                source.strip() == translated.strip():
            return [ReviewIssue(
                code="UNTRANSLATED", node_id=node_id,
                message="Translation equals source verbatim",
                severity="error",
            )]
        return []

    def review(self, node_id: str, source: str, translated: str,
               is_formula: bool = False,
               skip_formula_check: bool = False) -> ReviewResult:
        ...
        return ReviewResult(
            node_id=node_id, passed=not has_errors, issues=issues, action=action,
        )
...
class QualityPipeline:
    ...
    def __init__(self, reviewer: Optional[ReviewAgent] = None,
                 retranslator: Optional[Callable[[str], str]] = None) -> None:
        self.reviewer = reviewer or ReviewAgent()
        self.retranslator = retranslator

    def run(self, translated_map: Dict[str, Dict[str, str]],
            is_formula_map: Optional[Dict[str, bool]] = None) -> dict:
            ...
            if result.action == "retranslate" and self.retranslator is not None:
                ...
                if result2.passed:
                    ...
                    result.issues.append(ReviewIssue(
                        code="RETRANSLATE_FAILED", node_id=node_id,
