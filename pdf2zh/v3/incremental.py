...
class IncrementalEngine:
    ...
    def update(self, model: DocumentModel) -> dict:
        ...
        dirty = [bid for bid, h in current.items() if self._cache.get(bid) != h]
        cached = [bid for bid, h in current.items() if self._cache.get(bid) == h]
    ...
    def rebuild_plan(
        self, model: DocumentModel, dirty_ids: Optional[List[str]] = None
    ) -> List[dict]:
