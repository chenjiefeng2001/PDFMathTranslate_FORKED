...
class EventBus:
    ...
    def unsubscribe(self, t: EventType, h: EventHandler):
        if t in self._subscribers:
            self._subscribers[t] = [x for x in self._subscribers[t] if x is not h]

    def subscribe_all(self, h: EventHandler):
        self._wildcard.append(h)

    def unsubscribe_all(self, h: EventHandler):
        self._wildcard = [x for x in self._wildcard if x is not h]

    def emit(self, t: EventType, **kw) -> Event:
        e = Event(type=t, data=kw)
        self._dispatch(e)
        return e

    def emit_event(self, e: Event):
        self._dispatch(e)
    ...
    def subscriber_count(self, t=None):
        if t:
            return len(self._subscribers.get(t, []))
...
class NodeStateMachine:
    ...
    def set_state(self, nid, s):
        self._states[nid] = s

    def get_state(self, nid):
        return self._states.get(nid)

    def initialize(self, nid):
        self._states[nid] = NodeLifecycleState.NEW
    ...
    def transition(self, nid, to_state):
        ...
        if to_state not in _TRANSITIONS.get(cur, set()):
            return False
        old = cur
        self._states[nid] = to_state
        self._tc += 1
        for cb in self._callbacks:
            try:
                cb(nid, old, to_state)
            except Exception as e:
                logger.error("State cb failed: %s", e)
        return True
    ...
    def get_nodes_in_state(self, s):
        return [n for n, st in self._states.items() if st == s]
...
class DiagnosticCenter:
    ...
    def report(self, severity, module, message, *, node_id="", fix_hint=""):
        ...
        if len(self._diags) > self._max:
            self._diags = self._diags[-self._max :]
        return d

    def debug(self, m, msg, **kw):
        return self.report("debug", m, msg, **kw)

    def info(self, m, msg, **kw):
        return self.report("info", m, msg, **kw)

    def warning(self, m, msg, **kw):
        return self.report("warning", m, msg, **kw)

    def error(self, m, msg, **kw):
        return self.report("error", m, msg, **kw)

    def critical(self, m, msg, **kw):
        return self.report("critical", m, msg, **kw)

    def get_all(self):
        return list(self._diags)

    def get_by_severity(self, s):
        sev = DiagnosticSeverity(s.lower())
        return [d for d in self._diags if d.severity == sev]

    def get_by_module(self, m):
        return [d for d in self._diags if d.module == m]

    def get_by_node(self, nid):
        return [d for d in self._diags if d.node_id == nid]
    ...
    def summary(self):
        ...
        for sev in DiagnosticSeverity:
            ...
            if cnt:
                r[sev.value] = cnt
        r["total"] = len(self._diags)
        return r

    def to_dict_list(self):
        return [d.to_dict() for d in self._diags]
...
class MemoryCenter:
    ...
    def stats(self):
        return {
            "lookups": self._lk,
            "memory_hits": self._mh,
            "cache_hits": self._ch,
            "persistent_hits": self._ph,
            "misses": self._ms,
...
class Plugin:
    ...
    def on_error(self, ex):
        ...
        logger.error("Plugin %s error: %s", self.plugin_id, ex)
...
class PluginManager:
    ...
    def register(self, p: Plugin) -> str:
        if not p.plugin_id:
            raise ValueError("Plugin must have non-empty plugin_id")
        if p.plugin_id in self._plugins:
            raise ValueError(f"Plugin '{p.plugin_id}' already registered")
    ...
    def get_by_state(self, s):
        return [p for p in self._plugins.values() if p.state == s]
    ...
    def initialize_all(self, kernel):
        for pid, p in self._plugins.items():
            try:
                ...
                logger.error("Failed to init plugin %s: %s", pid, e)
                p.on_error(e)
        ...
        for pid, p in self._plugins.items():
            if p.state == PluginState.INITIALIZED:
                try:
                    ...
                    logger.error("Failed to start plugin %s: %s", pid, e)
                    p.on_error(e)
        self._started = True
    ...
    def running_count(self):
        return sum(1 for p in self._plugins.values() if p.state == PluginState.STARTED)
    ...
    def find_by_capability(self, cap: Capability) -> List[Plugin]:
        return [
            p
            for p in self._plugins.values()
            if isinstance(p, CapabilityPlugin) and cap in p.capabilities
        ]
    ...
    def get_best_for_capability(self, cap: Capability) -> Optional[Plugin]:
        ...
        return max(
            candidates,
            key=lambda p: (
                p.capability_priority if isinstance(p, CapabilityPlugin) else 0
            ),
        )
...
class RuntimeTransaction:
    ...
    def commit(self) -> dict:
        ...
        self._active = False
        result = {"nodes": len(self._snapshot.state) if self._snapshot else 0}
        self._snapshot = None
        return result
    ...
    def rollback(self) -> int:
        ...
        self._kernel.diagnostic_center._diags = [
            (
                Diagnostic(**d)
                if "module" in d
                else Diagnostic(severity=DiagnosticSeverity.INFO, module="", message="")
            )
...
class KnowledgeCenter:
    ...
    def learn(
        ...
        *,
        ...
        cross_refs: Optional[Set[str]] = None,
        confidence: float = 1.0,
    ) -> None:
        key = entity.lower()
        if key in self._entries:
            ...
            entry = KnowledgeEntry(
                entity=entity, definition=definition, confidence=confidence
            )
            self._entries[key] = entry
        if aliases:
            entry.aliases.update(a.lower() for a in aliases)
            for a in aliases:
                self._alias_index[a.lower()] = key
        if cross_refs:
            entry.cross_refs.update(c.lower() for c in cross_refs)
    ...
    def query_cross_references(self, name: str) -> List[str]:
        entry = self.query(name)
        return list(entry.cross_refs) if entry else []

    def resolve_alias(self, alias: str) -> Optional[str]:
        canonical = self._alias_index.get(alias.lower())
        return (
            self._entries[canonical].entity
...
class DiagnosticGraph:
    def __init__(self):
        self._nodes: Dict[str, DiagnosticNode] = {}
        self._edges: List[tuple] = []

    def add_diagnostic(self, d: Diagnostic) -> DiagnosticNode:
        import uuid as _uid

        nid = _uid.uuid4().hex[:12]
        node = DiagnosticNode(
            diagnostic_id=nid,
            severity=d.severity,
            module=d.module,
    ...
    def add_fix(self, diagnostic_id: str, fix_applied: str, success: bool) -> None:
        ...
        if node:
            node.repaired = success
            node.result = fix_applied

    def add_edge(
        self, source_id: str, target_id: str, relation: str = "causes"
    ) -> None:
    ...
    def get_chain(self, diagnostic_id: str) -> List[Dict]:
        ...
        if node:
            chain.append(
                {
                    "id": node.diagnostic_id,
                    "severity": node.severity.value,
                    "message": node.message,
                    "cause": node.cause,
                    "fix_hint": node.fix_hint,
    ...
    def resolve_count(self) -> int:
        return sum(1 for n in self._nodes.values() if n.repaired)
...
class TelemetryCollector:
    ...
    def summary(self) -> Dict[str, Any]:
        if not self._samples:
            return {"operations": {}, "total": 0, "error_rate": 0.0}
        ops = {}
        for s in self._samples:
            ...
            o = ops[s.operation]
            o["count"] += 1
            o["total_ms"] += s.duration_ms
            o["min_ms"] = min(o["min_ms"], s.duration_ms)
            o["max_ms"] = max(o["max_ms"], s.duration_ms)
            if not s.success:
                o["errors"] += 1
        for op in ops:
            c = ops[op]["count"]
            ops[op]["avg_ms"] = round(ops[op]["total_ms"] / c, 2) if c else 0
            ...
            ops[op]["p50_ms"] = (
                durs[
                    max(
                        0,
                        min(int(math.ceil(50 / 100.0 * len(durs))) - 1, len(durs) - 1),
                    )
                ]
                if durs
                else 0.0
            ...
            ops[op]["p95_ms"] = (
                durs[
                    max(
                        0,
                        min(int(math.ceil(95 / 100.0 * len(durs))) - 1, len(durs) - 1),
                    )
                ]
                if durs
                else 0.0
    ...
    def get_errors(self, operation: Optional[str] = None) -> Dict[str, int]:
        if operation:
            return {operation: self._errors.get(operation, 0)}
...
class RuntimeKernel:
    ...
    def service_locator(self):
        if self._sr is None:
            self._sr = ServiceRegistry.get_instance()
        return self._sr
    ...
    def shutdown(self):
        self.event_bus.emit(
            EventType.KERNEL_SHUTDOWN, kernel_id=self._kid, uptime=self.uptime
        )
        self.plugin_manager.stop_all()
        self._initialized = False

    def emit(self, t, **kw):
        return self.event_bus.emit(t, **kw)

    def on(self, t, h):
        self.event_bus.subscribe(t, h)

    def on_error(self, h):
        self.event_bus.subscribe(EventType.ERROR_OCCURRED, h)

    def initialize_node(self, nid):
        self.state_machine.initialize(nid)

    def transition_node(self, nid, state):
        ok = self.state_machine.transition(nid, state)
        if ok:
            self.event_bus.emit(EventType.NODE_UPDATED, node_id=nid, state=state.value)
        return ok

    def get_node_state(self, nid):
        return self.state_machine.get_state(nid)

    def diagnose(self, severity, module, message, *, node_id="", fix_hint=""):
        d = self.diagnostic_center.report(
            severity, module, message, node_id=node_id, fix_hint=fix_hint
        )
        if d.severity in (
    ...
    def get_service(self, iface):
        return self.service_locator.get(iface)
    ...
    # ── Knowledge ──────────────────────────────────────────────
    def learn(
        self, entity, definition="", *, aliases=None, cross_refs=None, confidence=1.0
    ):
        self.knowledge_center.learn(
            entity,
            definition,
            aliases=aliases,
    ...
    # ── Diagnostic Graph ──────────────────────────────────────
    def diagnose_graph(
        self, severity, module, message, *, node_id="", fix_hint="", cause=""
    ):
        ...
        *,
        ...
        if dependencies:
            for dep in dependencies:
                task.depends_on(dep)
        self._task_graph = (
            getattr(self, "_task_graph", None)
            or __import__("pdf2zh.v3.scheduler", fromlist=["TaskGraph"]).TaskGraph()
        )
    ...
    def stats(self):
        s = {
            ...
            "state_machine": {
                "nodes": self.state_machine.node_count,
                "transitions": self.state_machine.transition_count,
            },
            "diagnostic_center": self.diagnostic_center.summary(),
            "memory_center": self.memory_center.stats(),
            "plugin_manager": {
                "total": self.plugin_manager.plugin_count,
                "running": self.plugin_manager.running_count,
            },
            "services": self.service_locator.list_services(),
            "knowledge_center": {"entries": self.knowledge_center.entry_count()},
            "diagnostic_graph": {
                "total": self.diagnostic_graph.total_count(),
                "resolved": self.diagnostic_graph.resolve_count(),
            },
    ...
    def summary(self):
        ...
        return (
            ...
            % (
                self._kid,
                "RUNNING" if self._initialized else "STOPPED",
                s["uptime"],
                s["event_bus"]["total_events"],
                s["state_machine"]["nodes"],
                s["state_machine"]["transitions"],
                s["diagnostic_center"].get("total", 0),
                s["memory_center"]["hit_rate"] * 100,
                s["plugin_manager"]["total"],
                s["plugin_manager"]["running"],
            )
        )
