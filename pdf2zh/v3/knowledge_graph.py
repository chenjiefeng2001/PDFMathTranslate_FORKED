...
class PropagationReport:
    ...
    def merge(self, other: "PropagationReport") -> "PropagationReport":
        ...
        for name in ("entities_added", "entities_updated", "glossary_added",
                     "glossary_updated", "concepts_added", "citations_added"):
            setattr(self, name, getattr(self, name) + getattr(other, name))
        for name in ("total_entities", "total_glossary", "total_concepts",
                     "total_citations"):
...
class KnowledgeGraph:
    ...
    def _upsert_entity(self, name: str, info: dict, session_id: str,
                       report: PropagationReport) -> None:
        key = _normalize(name)
        ...
        canonical = str(info.get("canonical") or info.get("canonical_name")
                        or name)
        entity_type = str(info.get("type") or info.get("entity_type")
                          or "unknown")
        ...
        if existing is not None:
            ...
            self._entities[key] = KnowledgeEntity(
                name=name, canonical_name=canonical, entity_type=entity_type,
                aliases=aliases, definitions=definitions,
    ...
    def _upsert_glossary(self, source: str, target: str, session_id: str,
        ...
        if existing is not None:
            ...
            self._glossary[key] = GlossaryTerm(
                source=key, target=str(target), confidence=confidence,
                sessions=[session_id] if session_id else [])
    ...
    def _upsert_concept(self, name: str, info: dict, session_id: str,
                        report: PropagationReport) -> None:
        key = _normalize(name)
        parent = info.get("parent") if isinstance(info, dict) else None
        children = [str(c) for c in (info.get("children") or [])] \
    ...
    def _upsert_citation(self, citation_key: str, info: dict, session_id: str,
                         report: PropagationReport) -> None:
    ...
    def merge_analysis(self, analysis: Dict[str, Any],
                       session_id: str) -> PropagationReport:
        ...
        entity_map = entity_view.get("entities", entity_view) or {} \
            if isinstance(entity_view, dict) else {}
        ...
        concept_map = concept_view.get("concepts", concept_view) or {} \
            if isinstance(concept_view, dict) else {}
        ...
        citation_map = citation_view.get("citations", citation_view) or {} \
            if isinstance(citation_view, dict) else {}
    ...
    def merge_glossary(self, glossary: Any, session_id: str,
                       confidence: float = 1.0) -> PropagationReport:
        ...
        if isinstance(glossary, dict):
            ...
            for e in glossary.get_all_entries():
                pairs.append((str(getattr(e, "source_term", "")),
                              str(getattr(e, "target_term", ""))))
        elif isinstance(glossary, (list, tuple)):
            for item in glossary:
                if isinstance(item, dict):
                    pairs.append((str(item.get("source", "")),
                                  str(item.get("target", ""))))
                else:
                    pairs.append((str(item[0]), str(item[1])))
        for source, target in pairs:
            self._upsert_glossary(source, target, session_id, report,
    ...
    def merge(self, other: "KnowledgeGraph") -> PropagationReport:
        ...
        for ent in other._entities.values():
            self._upsert_entity(ent.name, {
                "canonical": ent.canonical_name, "type": ent.entity_type,
                "aliases": ent.aliases, "definitions": ent.definitions,
                "occurrences": ent.occurrence_count,
            }, "|".join(ent.sessions) or "merge", report)
        for term in other._glossary.values():
            self._upsert_glossary(term.source, term.target,
                                  "|".join(term.sessions) or "merge",
                                  report, confidence=term.confidence)
        for c in other._concepts.values():
            self._upsert_concept(c.name, {"parent": c.parent,
                                          "children": c.children},
                                 "|".join(c.sessions) or "merge", report)
        for cit in other._citations.values():
            self._upsert_citation(cit.citation_key, {"page": cit.page},
                                  "|".join(cit.sessions) or "merge", report)
        return report
    ...
    def session_ids(self) -> List[str]:
        ...
        sessions = set()
        for rec in (*self._entities.values(), *self._glossary.values(),
                    *self._concepts.values(), *self._citations.values()):
    ...
    def __len__(self) -> int:
        return (len(self._entities) + len(self._glossary) +
                len(self._concepts) + len(self._citations))
...
class KnowledgePropagator:
    ...
    def propagate(self, ctx: Any, session_id: str) -> PropagationReport:
        ...
        report = self.graph.merge_analysis(self.capture_analysis(ctx),
                                           session_id)
...
__all__ = [
    "KnowledgeEntity", "GlossaryTerm", "ConceptRecord", "CitationRecord",
    "PropagationReport", "KnowledgeGraph", "KnowledgePropagator",
]
