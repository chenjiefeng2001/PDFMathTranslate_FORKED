...
from pdf2zh.v3.visual_tree import VisualTree, VisualNode, VisualNodeType, BoundingBox, Page, Paragraph, Line, TextRun
...
class Renderer(ABC):
    @abstractmethod
    def render(self, tree: VisualTree, context: Optional[RenderContext] = None) -> bytes:
        pass
...
class PDFRenderer(Renderer):
    ...
    def render_page(self, page: Page, context=None):
        ctx = context or RenderContext()
        lines = [f"%PDF-page num={page.page_num} w={page.width:.0f} h={page.height:.0f}"]
        for node in page.walk():
            if isinstance(node, TextRun):
                lines.append(f"  text '{node.text[:40]}' @({node.bbox.x:.0f},{node.bbox.y:.0f}) font={node.font or 'default'} {len(node.text)}ch")
            elif isinstance(node, Line):
                lines.append(f"  line y={node.bbox.y:.0f} baseline={node.baseline:.0f} alignment={node.alignment or 'left'}")
            elif isinstance(node, Paragraph):
                lines.append(f"  para id={node.id} ({node.bbox.x:.0f},{node.bbox.y:.0f},{node.bbox.width:.0f}x{node.bbox.height:.0f}) lang={node.language or ''}")
...
class SVGRenderer(Renderer):
    ...
    def render(self, tree, context=None):
        ...
        for page in tree.pages:
            parts.append(f'<g id="page-{page.page_num}" transform="translate(0,{page.page_num * (page.height + 20)})">')
            parts.append(f'<rect x="0" y="0" width="{page.width}" height="{page.height}" fill="white" stroke="#ccc"/>')
            for node in page.walk():
                if isinstance(node, TextRun):
                    x = node.bbox.x if hasattr(node, "bbox") else 0
                    y = node.bbox.y if hasattr(node, "bbox") else 0
                    fs = node.font_size if hasattr(node, "font_size") and node.font_size else 12
                    safe = node.text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                    parts.append(f'<text x="{x:.1f}" y="{y + fs:.1f}" font-size="{fs}">{safe}</text>')
            parts.append("</g>")
        parts.append("</svg>")
        return "\n".join(parts).encode("utf-8")
    ...
    def render_page(self, page, context=None):
        ctx = context or RenderContext()
        parts = ['<?xml version="1.0" encoding="utf-8"?>']
        parts.append(f'<svg xmlns="http://www.w3.org/2000/svg" width="{page.width}" height="{page.height}">')
        parts.append(f'<rect x="0" y="0" width="{page.width}" height="{page.height}" fill="white"/>')
        for node in page.walk():
            if isinstance(node, TextRun):
                x = node.bbox.x if hasattr(node, "bbox") else 0
                y = node.bbox.y if hasattr(node, "bbox") else 0
        ...
        for page in tree.pages:
            for node in page.walk():
                if isinstance(node, Paragraph):
                    ...
                    p = doc.add_paragraph() if not hasattr(node, "_added_p") else doc.paragraphs[-1]
...
class RendererFactory:
    _registry = {"pdf": PDFRenderer, "html": HTMLRenderer, "markdown": MarkdownRenderer,
                          "svg": SVGRenderer, "docx": DOCXRenderer}

    @classmethod
    def create(cls, fmt: str) -> Renderer:
        renderer_cls = cls._registry.get(fmt.lower())
        if renderer_cls is None:
            raise ValueError(f"Unsupported format: {fmt}. Supported: {list(cls._registry.keys())}")
        return renderer_cls()
...
__all__ = ["RenderContext", "Renderer", "PDFRenderer", "HTMLRenderer",
