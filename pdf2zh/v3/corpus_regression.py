...
_KEYS = ("pages", "blocks", "formulas", "toc_entries", "headings",
         "tables", "references")


def expected_from_model(model: DocumentModel) -> Dict[str, int]:
    """从模型导出 Expected IR 桶计数（与诊断/报告口径一致）。"""
    counts: Dict[str, int] = {"pages": len(model.pages), "blocks": 0,
                              "formulas": 0, "toc_entries": 0,
                              "headings": 0, "tables": 0, "references": 0}
...
def compare_expected(expected: Dict[str, int],
                    actual: Dict[str, int]) -> Dict[str, Dict[str, int]]:
    changed = {}
    for key in _KEYS:
...
class RegressionReport:
    ...
    def to_dict(self) -> dict:
        return {"passed": self.passed_count, "failed": self.failed_count,
                "results": [r.to_dict() for r in self.results]}

    def summary(self) -> str:
        return (f"Regression passed={self.passed_count}/"
                f"{len(self.results)} failed={self.failed_count}")


def run_regression(cases: Sequence[tuple],
                   expected_map: Dict[str, Dict[str, int]]) -> RegressionReport:
    ...
    for name, model in cases:
        if name not in expected_map:
            report.results.append(RegressionResult(
                name, False, {"missing_expected": True}))
            continue
...
__all__ = ["_KEYS", "expected_from_model", "compare_expected",
