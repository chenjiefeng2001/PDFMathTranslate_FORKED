...
class DiagnosticReport:
    ...
    def summary(self) -> str:
        return (f"Diagnostics errors={self.error_count} "
...
def _unicode_issues(model, issue_list: List[DiagnosticIssue]) -> None:
    for page in model.pages:
        ...
        for i, block in enumerate(page.blocks):
            bad = [(g.decode, g.char) for l in block.lines for s in l.spans
                   for g in s.glyphs if g.decode != "ok"]
            if not bad and not any("\ufffd" in (l.text or "")
                                   for l in block.lines):
                continue
            issue_list.append(DiagnosticIssue(
                code="unicode_error", node_id=block_id(pno, i), page=pno,
                message=f"Unicode 损坏（{'�' if bad else 'fffd'} 字形 ×{len(bad) or 1}）",
                severity=CODE_SEVERITY["unicode_error"],
                evidence={"bad_glyphs": len(bad),
...
def _toc_issues(model, issue_list, toc_low_threshold: float = 0.6) -> None:
    for page in model.pages:
        ...
        for i, block in enumerate(page.blocks):
            ...
            if md.get("kind") == "toc":
                ...
                if conf < toc_low_threshold:
                    issue_list.append(DiagnosticIssue(
                        ...
                        node_id=block_id(pno, i), page=pno,
            ...
            if block.line_count >= 2 and len(dotted) >= 2 and \
                    (_RE_LEADER.search(text) or len(dotted) >= 3):
                issue_list.append(DiagnosticIssue(
                    code="toc_merged_lines", node_id=block_id(pno, i),
                    page=pno,
                    message="TOC 多行被合并成一段（Line Builder 阈值过大）",
                    severity=CODE_SEVERITY["toc_merged_lines"],
                    evidence={"lines": block.line_count,
                              "numbers": dotted[:5]}))
    ...
    for page in model.pages:
        ...
        for i, block in enumerate(page.blocks):
            ...
            if typo.get("overflow"):
                issue_list.append(DiagnosticIssue(
                    code="translation_overflow", node_id=block_id(pno, i),
                    page=pno, message="Translation overflow detected",
...
def _font_uncertain(model, issue_list) -> None:
    for page in model.pages:
        ...
        for i, block in enumerate(page.blocks):
            ...
            if len(fonts) > 1 and block.metadata.get("anomaly") == \
                    f"orphan_glyphs:{len(page.unassigned_glyphs)}":
                issue_list.append(DiagnosticIssue(
                    code="font_uncertain", node_id=block_id(pno, i),
                    page=pno, message="Font mapping uncertain (multifont)",
    ...
    for page in model.pages:
        ...
        for i, block in enumerate(page.blocks):
            if block.metadata.get("anomaly") == "empty_text":
                issue_list.append(DiagnosticIssue(
                    code="empty_block", node_id=block_id(pno, i), page=pno,
                    message="Empty text block detected",
                    severity=CODE_SEVERITY["empty_block"],
                    evidence={"bbox": list(block.bbox)}))


def analyze_document(model: DocumentModel,
                     toc_low_threshold: float = 0.6,
                     formula_threshold: float = 0.5) -> DiagnosticReport:
...
_KIND_BASE = {
    "toc": 0.55, "heading": 0.65, "caption": 0.6, "formula": 0.6,
    "footnote": 0.6, "paragraph": 0.85, "code": 0.7, "table": 0.7,
    "figure": 0.9, "header": 0.8, "footer": 0.8,
}
...
def node_confidence(block) -> tuple:
    ...
    if any(g.decode != "ok" for l in block.lines for s in l.spans
           for g in s.glyphs):
        conf *= 0.3
