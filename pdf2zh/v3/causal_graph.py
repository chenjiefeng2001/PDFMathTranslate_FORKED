...
class CausalNode:
    ...
    def add_cause(self, cause_id):
        self.cause_ids.add(cause_id)

    def add_effect(self, effect_id):
...
class CausalDiagnosticGraph:
    ...
    def add_causal_chain(self, chain):
        ...
        for label, severity, module in chain:
            n = self.add_diagnostic(
                label,
                severity=severity,
                module=module,
                cause_ids=[prev_id] if prev_id else None,
            )
            nodes.append(n)
            prev_id = n.id
        return nodes
    ...
    def remove_node(self, nid):
        ...
        if n:
            for cid in list(n.cause_ids):
                if cid in self._nodes:
                    self._nodes[cid].effect_ids.discard(nid)
            for eid in list(n.effect_ids):
                if eid in self._nodes:
                    self._nodes[eid].cause_ids.discard(nid)

    def find_root_causes(self):
        return [n for n in self._nodes.values() if n.is_root_cause]

    def get_leaf_causes(self):
        return [n for n in self._nodes.values() if not n.effect_ids]
    ...
    def mark_repaired(self, nid, result=""):
        ...
        if n:
            n.repair_status = RepairStatus.APPLIED
            n.repair_result = result

    def mark_failed(self, nid, result=""):
        n = self._nodes.get(nid)
        if n:
            n.repair_status = RepairStatus.FAILED
            n.repair_result = result
    ...
    def get_unresolved(self):
        return [
            n for n in self._nodes.values() if n.repair_status != RepairStatus.APPLIED
        ]
    ...
    def stats(self):
        ...
        for n in self._nodes.values():
            ...
            if n.module:
                by_module[n.module] = by_module.get(n.module, 0) + 1
        return {
            "total": self.node_count,
            "root_causes": len(self.find_root_causes()),
