...
const SPA_OVERLAY = {
  "zh-CN": {
    ui: {
      ...
      config_ingest_backend: "摄入后端（MinerU / Marker / Jina）",
      config_ingest_backend_info:
        "PDF 理解（ingestion）后端：auto=自动选择，mineru=MinerU，marker=Marker，jina=Jina OCR。Jina 权重为 CC BY-NC 4.0，商业使用需另行取得授权。",
      config_ingest_auto: "自动选择",
      config_ingest_mineru: "MinerU",
      config_ingest_marker: "Marker",
      config_ingest_jina: "Jina OCR",
      config_jina_default: "使用默认",
      config_jina_model: "Jina 模型",
      config_jina_model_info: "Jina OCR 模型 ID；留空使用后端默认模型。",
      config_jina_revision: "Jina 模型版本",
      ...
      config_jina_device_cuda: "CUDA",
      config_jina_dpi: "Jina 渲染 DPI",
      config_jina_dpi_info: "PDF 页面渲染 DPI；数值越高图像越清晰，但内存和处理时间也会增加。",
      config_jina_max_pixels: "Jina 每页最大像素",
      config_jina_max_pixels_info: "每页渲染图像的最大像素数。",
      config_jina_max_new_tokens: "Jina 最大生成 tokens",
      config_jina_max_new_tokens_info: "Jina OCR 单次生成的最大 token 数。",
      config_jina_timeout: "Jina 超时时间（秒）",
      config_jina_timeout_info: "Jina OCR 任务的最长等待时间，单位为秒。",
      config_jina_cache_dir: "Jina 缓存目录",
      config_jina_cache_dir_info: "Jina OCR 的本地结果缓存目录；留空关闭磁盘缓存。",
      config_jina_min_coverage: "Jina 最低覆盖率",
      config_jina_min_coverage_info: "结果覆盖率低于此阈值时判定为失败。",
      config_jina_offline: "Jina 离线模式",
      config_jina_offline_info: "仅使用本地已缓存的模型和依赖，不访问网络。",
      ...
      settings_jina: "Jina OCR 隔离环境",
      settings_jina_hint:
        "Jina 模型运行在独立 Python 环境中；安装依赖后，首次使用 Jina 时模型权重会下载到本机缓存。",
      settings_jina_ready: "Jina OCR 环境已就绪",
      settings_jina_install: "一键安装 Jina 环境",
      settings_jina_installing: "正在安装 Jina 环境（下载 torch 等，请稍候）",
      settings_copy: "复制命令",
      settings_copied: "已复制",
      download_done: "已下载",
  ...
  en: {
    ui: {
      ...
      config_ingest_backend: "Ingestion backend (MinerU / Marker / Jina)",
      config_ingest_backend_info:
        "PDF-understanding backend: auto selects automatically; mineru, marker, and jina are explicit. Jina weights are CC BY-NC 4.0; commercial use requires separate permission.",
      config_ingest_auto: "Automatic selection",
      config_ingest_mineru: "MinerU",
      config_ingest_marker: "Marker",
      config_ingest_jina: "Jina OCR",
      config_jina_default: "Use default",
      config_jina_model: "Jina model",
      config_jina_model_info: "Jina OCR model ID; leave empty to use the backend default.",
      config_jina_revision: "Jina model revision",
      ...
      config_jina_device_cpu: "CPU",
      config_jina_device_cuda: "CUDA",
      config_jina_dpi: "Jina render DPI",
      config_jina_dpi_info: "PDF page rendering DPI; higher values improve image detail but increase memory use and processing time.",
      config_jina_max_pixels: "Jina max pixels per page",
      config_jina_max_pixels_info: "Maximum pixels for each rendered page.",
      config_jina_max_new_tokens: "Jina max generated tokens",
      config_jina_max_new_tokens_info: "Maximum number of tokens generated by one Jina OCR request.",
      config_jina_timeout: "Jina timeout (seconds)",
      config_jina_timeout_info: "Maximum time to wait for a Jina OCR task, in seconds.",
      config_jina_cache_dir: "Jina cache directory",
      config_jina_cache_dir_info: "Local result cache for Jina OCR; leave empty to disable disk caching.",
      config_jina_min_coverage: "Jina minimum coverage",
      config_jina_min_coverage_info: "Results with coverage below this threshold are treated as failures.",
      config_jina_offline: "Jina offline mode",
      config_jina_offline_info: "Use only locally cached models and dependencies without network access.",
      ...
      settings_jina: "Jina OCR isolated environment",
      settings_jina_hint:
        "Jina runs in a separate Python environment. After installing dependencies, the model weights download into the local cache on first use.",
      settings_jina_ready: "Jina OCR environment ready",
      settings_jina_install: "Install Jina environment",
      settings_jina_installing: "Installing Jina environment (downloading torch, please wait)",
