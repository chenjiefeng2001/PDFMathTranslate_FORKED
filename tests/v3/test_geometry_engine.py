...
from pdf2zh.v3.geometry import (
    Char, GeometryConfig, GeometryEngine, Line, PageGeometry, Paragraph,
    Word, extract_chars_from_page,
)


def _mk_chars(text, x, y0, size=10.0, gap=1.0, font="Helvetica",
              page=0, char_w=None):
    ...
    for ch in text:
        chars.append(Char(text=ch, x0=cx, y0=y0, x1=cx + cw, y1=y0 + size,
                          size=size, font=font, page_num=page))
        cx += cw + gap
    return chars
...
class TestWordClustering:
    ...
    def test_no_space_pdf_falls_back_to_gap_heuristic(self):
        ...
        far = Char(text="C", x0=100.0 + 8 * 10, y0=700.0, x1=100.0 + 8 * 10 + 5,
                   y1=700.0 + 10, size=10.0)
...
class TestLineBuilding:
    def test_words_same_baseline_join_line(self):
        words = [Word([Char("A", 100, 700, 105, 710)]),
    ...
    def test_column_gap_splits_line(self):
        ...
        lines = GeometryEngine().build_lines(
            GeometryEngine().build_words(left + right))
        assert len(lines) == 2
...
class TestReadingOrder:
    ...
    def test_toc_page_with_page_number(self):
        ...
        page = self._page(paras)
        assert [p.text for p in page.reading_order()] == [
            "Chapter 1 Intro...3", "Chapter 2 Methods...12", "42",
        ]
    ...
    def test_mixed_title_two_columns_toc(self):
        ...
        # 双栏：L 全部先于 R
        assert order.index("L1") < order.index("L2") < order.index("R1") < order.index("R2")
        # 目录与页码在栏之后
...
class TestConfig:
    def test_custom_word_gap(self):
        ...
        chars = _mk_chars("AB", 100.0, 700.0)
        far = Char(text="C", x0=100.0 + 2 * 10, y0=700.0,
                   x1=100.0 + 2 * 10 + 5, y1=700.0 + 10, size=10.0)
    ...
    def test_column_split_threshold_is_font_relative(self):
        ...
        words = [Word([Char("A", 100, 700, 104, 708, size=8.0)]),
        ...
        words = [Word([Char("A", 100, 700, 108, 716, size=16.0)]),
