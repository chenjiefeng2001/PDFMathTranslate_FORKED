...
class TestLLMProvider(unittest.TestCase):
    ...
    def test_mock_provider_complete(self):
        from pdf2zh.v3.translator import MockLLMProvider
        p = MockLLMProvider()
        resp = p.complete([{"role": "user", "content": "Text to translate: Hello world"}])
    ...
    def test_mock_provider_with_model(self):
        from pdf2zh.v3.translator import MockLLMProvider
        p = MockLLMProvider()
        resp = p.complete([{"role": "user", "content": "Text to translate: x"}], model="gpt-4o")
...
class TestPostProcessor(unittest.TestCase):
    ...
    def test_whitespace_normalization(self):
        self.assertEqual(self.pp.process("  Line1\n  \nLine2  ").text, "Line1\n  \nLine2")
    ...
    def test_with_memory_glossary_mismatch(self):
        ...
        node = DocumentNode(id="n1", node_type=NodeType.PARAGRAPH, bbox=B, text="PDF file format")
...
class TestTranslationStats(unittest.TestCase):
    ...
    def test_merge(self):
        from pdf2zh.v3.translator import TranslationStats
        a = TranslationStats(total_nodes=10, translated=8, total_latency_ms=100, total_tokens=500)
    ...
    def test_to_dict(self):
        from pdf2zh.v3.translator import TranslationStats
        d = TranslationStats(total_nodes=5, translated=4, total_latency_ms=200, total_tokens=100).to_dict()
