...
def make_document_graph() -> DocumentGraph:
    ...
    g.add_node(
        DocumentNode(
            id="page_1", node_type=NodeType.PAGE, bbox=(0, 0, 612, 792), page_num=1
        )
    )
    rows = [
    ...
    for i, (nid, text, ntype, x, y) in enumerate(rows):
        g.add_node(
            DocumentNode(
                id=nid, node_type=ntype, text=text, bbox=(x, y, 300, 20), page_num=1
            )
        )
...
def make_ir():
    ...
    return IRBuilder(title="agent_test", source_lang="en", target_lang="zh-cn").build(g)
...
def test_parser_flags_missing_text():
    ...
    ir = IRBuilder(title="t", source_lang="en", target_lang="zh-cn").build(g)
...
def test_pipeline_converges_with_strict_feedback():
    ...
    pipeline = AgentPipeline(
        translator=translator, glossary={"LLM": "大语言模型"}, max_feedback_rounds=2
    )
...
def test_pipeline_does_not_hang_on_unfixable():
    ...
    pipeline = AgentPipeline(
        translator=never_fix, glossary={"LLM": "大语言模型"}, max_feedback_rounds=2
    )
    report = pipeline.run(make_ir())
