...
from pdf2zh.v3.mainline_gate import (
    GateBlock, GatedResult, MainlineRelayoutGate, _blocks_overlap_rate,
)


def _blocks():
    return [
        GateBlock(node_id="a", text="First block", x=72, y=100,
                  width=400, height=20, page=1),
        GateBlock(node_id="b", text="Second block", x=72, y=130,
                  width=400, height=20, page=1),
    ]
...
def test_gate_blocks_writeback_when_overflow_persists():
    blocks = [GateBlock(node_id="x", text="X", x=72, y=780,
                        width=400, height=100, page=1)]
...
def test_gate_max_passes_zero_never_relayouts():
    blocks = [
        GateBlock(node_id="a", text="A", x=72, y=100, width=400,
                  height=20, page=1),
        GateBlock(node_id="b", text="B", x=72, y=120, width=400,
                  height=20, page=1),
    ]
...
def test_gate_kept_roles_keep_geometry():
    formula = GateBlock(node_id="f", text="E = mc^2", x=72, y=200,
                        width=300, height=20, page=1,
                        node_type="formula")
    result = MainlineRelayoutGate().run([formula],
                                        translations={"f": "E = mc^2"})
    assert result.writeback_allowed
    out = result.blocks[0]
