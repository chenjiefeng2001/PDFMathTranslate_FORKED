...
from pdf2zh.services.runtime_service import (
    RuntimeService, ServiceConfig, TaskStage, TranslationRequest,
)
...
class TestFeatureFlagSync:
    ...
    def test_sync_v4_enabled(self):
        svc = RuntimeService(config=ServiceConfig(use_v4_engine=True,
                                                  use_v4_layout=True))
    ...
    def test_sync_per_component(self):
        svc = RuntimeService(config=ServiceConfig(use_v4_engine=False,
                                                  use_v4_repair=True))
...
class TestRunEvaluationConfig:
    ...
    def test_evaluation_metrics_shape(self):
        ...
        report = EvaluationReport(
            geometry={"geometry_score": 95.0}, structure={},
...
class TestV4GateConfigSync:
    ...
    def test_gate_factory_guarded(self, monkeypatch):
        import pdf2zh.services.runtime_service as rs_mod
        monkeypatch.delitem(__import__("sys").modules, "pdf2zh.v3.mainline_gate",
                            raising=False)
...
class TestTaskStateSideChannel:
    def test_task_state_fields_exist(self):
        svc = RuntimeService()
        tid = svc.submit_task(TranslationRequest(source_path="",
                                                 target_lang="zh-CN"))
...
class TestCharsFromLTPage:
    def test_adapts_pdfminer_chars(self):
        ...
        ltp = type("LTPage", (), {"_objs": [cont], "bbox": (0, 0, 612, 792),
