...
def _chain_graph():
    ...
    cg.add_edge("a", "b", ConstraintRelation.MUST_BELOW, priority="hard", gap=10.0)
    cg.add_edge("b", "c", ConstraintRelation.MUST_BELOW, priority="hard", gap=10.0)
    return cg
...
def test_advisory_edges_are_not_enforced():
    ...
    cg.add_edge(
        "a", "b", ConstraintRelation.MUST_BELOW, priority="typography", gap=500.0
    )
    solver = ConstraintSolver(cg, 612, 792)
...
def test_composite_score_uses_weights():
    ...
    assert composite_score(r) == pytest.approx(
        WEIGHTS["translation"] * 95.0
        + WEIGHTS["semantic"] * 90.0
        + WEIGHTS["typography"] * 88.0
        + WEIGHTS["layout"] * 92.0
        + WEIGHTS["consistency"] * 91.0,
        abs=0.01,
    )
...
def test_quality_gate_fail_and_per_page():
    low = EvaluationResult(
        translation_score=60.0,
        semantic_score=70.0,
        typography_score=80.0,
        layout_score=90.0,
...
def test_quality_snapshot_roundtrip(tmp_path):
    snap = QualitySnapshot.capture(
        _sample_result(), total_score=91.5, extra={"doc": "x"}
    )
...
def test_feature_flags_evaluate_with_policy():
    flags = FeatureFlags(
        use_v4_engine=False,
        rollout_policy=RolloutPolicy().add(PageRangeRolloutRule([1])),
    )
...
def test_typography_rule_adopts_geometry():
    rule = TypographyRule()
    m = rule.apply(
        "Short translation", source="Short source", bbox=(0, 0, 400, 20), font_size=12.0
    )
...
def test_typography_rule_shrinks_font_on_overflow():
    ...
    m = rule.apply(long_zh, source="src", bbox=(0, 0, 120, 20), font_size=12.0)
...
def test_typography_rule_expands_block():
    rule = TypographyRule()
    m = rule.apply(
        "中文", source="English source text", bbox=(0, 0, 400, 8), font_size=12.0
    )
