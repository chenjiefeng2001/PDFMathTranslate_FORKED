...
def make_img(image_class=ImageClass.DIAGRAM, regions=None):
    obj = ImageObject(id="x", image_class=image_class, features={"color_count": 8})
...
class TestImagePipelineOCRFeed(unittest.TestCase):
    ...
    def test_ocr_backfill_feeds_decision(self):
        ...
        img._pixels = pixels
        d = decide_with_ocr(img, ocr_backend=DeterministicOCRBackend(narrow_text="42"))
        by_bbox = {
            tuple(round(v, 2) for v in r.region.bbox): r for r in d.region_decisions
        }
    ...
    def test_analyze_failure_degrades_to_preserve(self):
        ...
        with patch(
            "pdf2zh.v3.image_pipeline.analyze_image_bytes",
...
class TestRenderTakeover(unittest.TestCase):
    ...
    def test_plan_routes_overflow_to_block_when_rejected(self):
        ...
        self.assertEqual(plan["routing"]["p0_1"]["render_path"], "translate_refit")
    ...
    def test_untranslated_flagged_retranslate(self):
        ...
        report = run_translation_qa(
            [
                {
                    "node_id": "p0_0",
                    "text": "This is a long sentence here",
    ...
    def test_numbers_missing_flagged_warning(self):
        ...
        report = run_translation_qa(
            [
                {
                    "node_id": "p0_0",
                    "text": "Table 3 shows results of 42 runs",
