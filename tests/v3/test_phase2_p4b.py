...
class TestEvaluationIssueMapper(unittest.TestCase):
    def test_no_issues(self):
        ...
        r = EvaluationResult(translation_score=95, semantic_score=90,
                             typography_score=85, layout_score=95, consistency_score=90)
        self.assertEqual(EvaluationIssueMapper.map_result(r).total, 0)
    def test_low_translation(self):
        from pdf2zh.v3.evaluator import EvaluationResult, EvaluationIssueMapper
        r = EvaluationResult(translation_score=50, semantic_score=85,
                             typography_score=85, layout_score=85, consistency_score=85)
        issues = EvaluationIssueMapper.map_result(r).get_by_module("translation")
        self.assertGreater(len(issues), 0)
    def test_low_layout_critical(self):
        from pdf2zh.v3.evaluator import EvaluationResult, EvaluationIssueMapper, IssueSeverity
        r = EvaluationResult(translation_score=95, semantic_score=95,
                             typography_score=95, layout_score=40, consistency_score=95)
...
class TestQualityEvaluatorDiagnostic(unittest.TestCase):
    def test_diagnostic_on_result(self):
        ...
        g = DocumentGraph()
        g.add_node(DocumentNode(id="n1", node_type=NodeType.PARAGRAPH, bbox=B, text="test"))
