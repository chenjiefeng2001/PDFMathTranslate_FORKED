...
class TestSession(unittest.TestCase):
    ...
    def test_translator_uses_session(self):
        ...
        graph = DocumentGraph()
        graph.add_node(DocumentNode(id="n1", node_type=NodeType.PARAGRAPH, bbox=B, text="test"))
    ...
    def test_translate_all(self):
        from pdf2zh.v3.translator import TranslationSession, Translator
        graph = DocumentGraph()
        graph.add_node(DocumentNode(id="n1", node_type=NodeType.PARAGRAPH, bbox=B, text="Hello"))
        graph.add_node(DocumentNode(id="n2", node_type=NodeType.PARAGRAPH, bbox=B, text="World"))
        s = TranslationSession(graph=graph)
        t = Translator(s)
...
class TestInlineLayout(unittest.TestCase):
    ...
    def test_break_line_overflow(self):
        from pdf2zh.v3.layout import InlineLayout
        self.assertGreater(len(InlineLayout.break_line("Hello World Test", 50, 12.0)), 1)
...
class TestColumnLayout(unittest.TestCase):
    def test_single(self):
        ...
        n = DocumentNode(id="n1", node_type=NodeType.PARAGRAPH, bbox=(0, 0, 100, 50), text="x")
        self.assertEqual(ColumnLayout.detect_columns([n], 612)[0], 1)

    def test_two(self):
        n1 = DocumentNode(id="n1", node_type=NodeType.PARAGRAPH, bbox=(50, 100, 250, 150), text="left")
        n2 = DocumentNode(id="n2", node_type=NodeType.PARAGRAPH, bbox=(350, 100, 550, 150), text="right")
        from pdf2zh.v3.layout import ColumnLayout
    ...
    def test_assign_left(self):
        ...
        n = DocumentNode(id="n1", node_type=NodeType.PARAGRAPH, bbox=(60, 100, 160, 150), text="x")
    ...
    def test_assign_right(self):
        from pdf2zh.v3.layout import ColumnLayout, ColumnRegion
        n = DocumentNode(id="n1", node_type=NodeType.PARAGRAPH, bbox=(400, 100, 500, 150), text="x")
        cols = [ColumnRegion(x=50, w=200), ColumnRegion(x=350, w=200)]
