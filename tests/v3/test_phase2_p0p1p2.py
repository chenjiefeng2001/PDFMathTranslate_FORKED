...
class TestModelRouter(unittest.TestCase):
    ...
    def test_default_route(self):
        r = self.router.route(
            DocumentNode(id="n1", node_type=NodeType.PARAGRAPH, bbox=B, text="x")
        )
    ...
    def test_formula_route_cold(self):
        r = self.router.route(
            DocumentNode(id="n2", node_type=NodeType.FORMULA, bbox=B, text="E=mc^2")
        )
    ...
    def test_code_route_cold(self):
        r = self.router.route(
            DocumentNode(id="n3", node_type=NodeType.CODE, bbox=B, text="print(1)")
        )
        self.assertEqual(r.temperature, 0.0)

    def test_footnote_route(self):
        r = self.router.route(
            DocumentNode(id="n4", node_type=NodeType.FOOTNOTE, bbox=B, text="Note")
        )
        self.assertEqual(r.model, "gpt-4o-mini")
    ...
    def test_abstract_route(self):
        r = self.router.route(
            DocumentNode(id="n5", node_type=NodeType.ABSTRACT, bbox=B, text="Abstract")
        )
        self.assertEqual(r.model, "gpt-4o")
        self.assertEqual(r.max_tokens, 8192)
...
class TestTranslationCache(unittest.TestCase):
    ...
    def test_put_and_get(self):
        ...
        self.assertEqual(
            self.cache.get("Hello", "en", "zh-cn", "gpt-4o"), "\u4f60\u597d"
        )
...
class TestPromptComposer(unittest.TestCase):
    ...
    def test_compose_roles(self):
        g = DocumentGraph()
        g.add_node(
            DocumentNode(id="n2", node_type=NodeType.HEADING, bbox=B, text="Intro")
        )
    ...
    def test_compose_includes_glossary(self):
        ...
        g.add_node(
            DocumentNode(
                id="n3", node_type=NodeType.PARAGRAPH, bbox=B, text="PDF format"
            )
        )
...
class TestTranslationSession(unittest.TestCase):
    ...
    def test_apply_results(self):
        ...
        self.assertEqual(
            self.graph.get_node("n1").translated_text, "\u4f60\u597d\u4e16\u754c"
        )
...
class TestTranslator(unittest.TestCase):
    def setUp(self):
        ...
        for i in range(3):
            self.graph.add_node(
                DocumentNode(
                    id=f"n{i}", node_type=NodeType.PARAGRAPH, bbox=B, text=f"Text {i}"
                )
            )
...
class TestMeasure(unittest.TestCase):
    ...
    def test_cjk_width(self):
        self.assertAlmostEqual(
            self.M.measure_text("\u4f60\u597d\u4e16\u754c"), 48.0, delta=1.0
        )
...
class TestConstraintSolver(unittest.TestCase):
    def setUp(self):
        ...
        self.c1 = LayoutConstraint(
            ConstraintType.SOFT, "a", "b", "must_follow", gap=10.0
        )
...
class TestPDFRenderer(unittest.TestCase):
    ...
    def test_render_page(self):
        ...
        line = Line(
            id="pl1", bbox=BoundingBox(50, 55, 500, 16), baseline=57, line_height=16
        )
...
class TestIssueAndGraph(unittest.TestCase):
    ...
    def test_issue_graph_add(self):
        ...
        g.add_issue(
            Issue(
                "overlap", IssueSeverity.MAJOR, "overlap", node_id="n1", module="layout"
            )
        )
    ...
    def test_issue_graph_multiple(self):
        ...
        g.add_issues(
            [
                Issue("a", IssueSeverity.CRITICAL, "a", module="m1"),
                Issue("b", IssueSeverity.INFO, "b", module="m2"),
            ]
        )
...
class TestRepairScheduler(unittest.TestCase):
    ...
    def test_schedule_all(self):
        ...
        g.add_issue(
            Issue("bad_translation", IssueSeverity.MAJOR, "bt", module="translation")
        )
        rs = RepairScheduler()
...
class TestLayoutOptimizer(unittest.TestCase):
    ...
    def test_set_elements(self):
        ...
        self.assertIn("b", r.positions)

    def test_optimize_y_positions(self):
        from pdf2zh.v3.optimizer import LayoutElement

        self.opt.set_elements(
            [LayoutElement("x", 100, 50), LayoutElement("y", 100, 30)]
        )
        r = self.opt.optimize()
    ...
    def test_optimize_follows_constraints(self):
        ...
        self.opt.set_elements(
            [LayoutElement("a", 100, 50), LayoutElement("b", 100, 50)]
        )
        self.opt.add_constraint(
            LayoutConstraint(ConstraintType.HARD, "a", "b", "must_below", gap=10.0)
        )
        r = self.opt.optimize()
