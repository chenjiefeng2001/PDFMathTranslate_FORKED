...
try:
    from pdf2zh.v3.legacy_adapter import (
        AdapterStats, BaseAdapter, LegacyTranslatorAdapter,
...
class TestLegacyTranslatorAdapterWithGraph(unittest.TestCase):
    def _make_graph_with_text(self):
        g = DocumentGraph()
        g.add_node(DocumentNode("p1", NodeType.PARAGRAPH, (0,0,200,30), text="Test para 1.", page_num=0))
        g.add_node(DocumentNode("p2", NodeType.PARAGRAPH, (0,40,200,70), text="Test para 2.", page_num=0))
        g.add_node(DocumentNode("h1", NodeType.HEADING, (0,80,200,100), text="Test Heading", page_num=0))
        return g
