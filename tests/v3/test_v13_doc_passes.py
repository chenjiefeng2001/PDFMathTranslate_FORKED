...
class TestPassManager(unittest.TestCase):
    def test_run_and_report(self):
        from pdf2zh.v3.doc_passes import (
            NormalizePass, PassManager, SemanticPass, TranslationPolicyPass,
        )
        model = build_model()
        mgr = PassManager([NormalizePass(), SemanticPass(),
    ...
    def test_pass_diff_records_changes(self):
        ...
        # code 块 kind 变化应被记录
        changed = [d for d in diff if d.field == "kind"
                   and d.before != d.after]
...
class TestNormalizePass(unittest.TestCase):
    ...
    def test_normalize_pass_writes_reading_order_and_anomaly(self):
        ...
        model = build_model()
        model.pages[0].blocks.append(BlockModel(
            text="Bad  text\u200b here", kind="paragraph",
            x0=0, y0=0, x1=100, y1=10))
        model.pages[0].blocks[-1].lines = [
            LineModel(text="Bad  text\u200b here")]
        ...
        # 空文本块 → 异常标记
        p1.blocks.append(BlockModel(
            text="", kind="paragraph", x0=0, y0=0, x1=10, y1=10))
...
class TestSemanticPass(unittest.TestCase):
    ...
    def test_detect_helpers(self):
        ...
        from pdf2zh.v3.canonical_page import BlockModel, LineModel, SpanModel, GlyphModel
        code = BlockModel(text="def foo(): pass", kind="paragraph",
                          x0=0, y0=0, x1=100, y1=10)
        code.lines = [LineModel(text="def foo(): pass")]
        self.assertTrue(detect_code_block(code))
        tbl = BlockModel(text="a  b  c\n1  2  3", kind="paragraph",
                         x0=0, y0=0, x1=200, y1=20)
...
class TestTranslationPolicyPass(unittest.TestCase):
    def test_policy_per_kind(self):
        from pdf2zh.v3.doc_passes import (
            SemanticPass, TranslationPolicyPass, translation_policy_for,
        )
        model = build_model()
        ...
        # 正文 → translate
        body = [p for p in by_kind.values()
                if p.get("reason") == "body"]
    ...
    def test_toc_source_text_is_title(self):
        ...
        toc = BlockModel(text="5.2.1 Parser ...... 292", kind="toc",
    ...
    def test_translate_document_honors_policy(self):
        from pdf2zh.v3.doc_passes import (
            SemanticPass, TranslationPolicyPass,
        )
        model = build_model()
        SemanticPass().run(model)
        TranslationPolicyPass().run(model)
...
class TestTypographyPass(unittest.TestCase):
    def test_typography_engine_functions(self):
        from pdf2zh.v3.typography_engine import (
            line_break, measure, justify_advances,
        )
    ...
    def test_typography_pass_marks_overflow(self):
        ...
        p1 = model.pages[0]
        typed = [b.metadata["typography"] for b in p1.blocks
                 if b.metadata.get("typography")]
