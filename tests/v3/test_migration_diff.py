...
def test_normalize_block_from_dict():
    b = normalize_block({"id": "n1", "text": "hi", "page_num": 2,
                         "bbox": (10, 20, 110, 40)})
...
def _blocks_a():
    return [
        {"id": "p1", "text": "Introduction to machine learning",
         "page_num": 1, "bbox": (72, 72, 372, 92)},
        {"id": "p2", "text": "A second paragraph", "page_num": 1,
         "bbox": (72, 120, 372, 140)},
        {"id": "p3", "text": "Conclusion", "page_num": 2,
         "bbox": (72, 72, 372, 92)},
    ]
...
def test_harness_page_drift():
    legacy = _blocks_a()
    v4 = _blocks_a() + [{"id": "extra", "text": "x", "page_num": 9,
                         "bbox": (0, 0, 10, 10)}]
    report = MigrationDiffHarness().compute(legacy, v4)
    assert not report.passed
...
def test_harness_custom_thresholds():
    ...
    report = MigrationDiffHarness(thresholds={"bbox_displacement": 1000.0}
                                  ).compute(blocks, blocks)
    assert report.passed


def test_harness_bbox_displacement_regression():
    legacy = _blocks_a()
    v4 = [dict(b, bbox=(b["bbox"][0], b["bbox"][1] + 200, *b["bbox"][2:]))
          for b in legacy]
    report = MigrationDiffHarness().compute(legacy, v4)
    assert not report.passed
...
def test_ir_roundtrip_baseline():
    ...
    ir = IRBuilder(title="roundtrip", source_lang="en",
...
def test_snapshot_ir_buckets():
    ...
    for bucket in ("paragraphs", "captions", "tables", "headings",
...
def test_corpus_covers_layout_families():
    ...
    families = [corpus.TEMPLATES[i % len(corpus.TEMPLATES)]
                for i in range(3)]
    # textbook family exposes formulas; paper family exposes references
    assert any(s["formulas"] for s, f in zip(snaps, families)
               if f == "textbook")
