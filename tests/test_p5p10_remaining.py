...
def _mk_formula_unit(x0=48, width=300):
    ...
    formula = FormulaObject(
        ...
        bbox=(x0, 97, x0 + 7, 111), baseline=100)
    return TranslationUnit(
        unit_id="u0", page_id=0,
        source_text_with_anchors="Let <formula_0> be.",
        formula_map={"<formula_0>": formula},
        source_bbox=(10, 96, width, 112),
        inline_structure=[
            InlineTextRun(text="Let ", style_runs=[], bbox=(10, 96, 40, 112),
                          font_size=12),
            formula,
            InlineTextRun(text="be.", style_runs=[], bbox=(60, 96, 80, 112),
                          font_size=12),
        ],
        master_baseline=100, source_line_baselines=[100],
    )
...
class TestLayoutClassInjection:
    def test_mask_sampling(self):
        ...
        cls_fn = layout_mask_class_fn(
            SimpleNamespace(layout={0: mask}), None)
        assert cls_fn(paras[0]) == 0

    def test_mask_missing_returns_none(self):
        from types import SimpleNamespace
        paras = build_logical_paragraphs(
            VisualLineBuilder().build([mk_glyph("A", 0, 100, 12)], page_id=0),
            page_id=0)
        assert layout_mask_class_fn(
            SimpleNamespace(layout={}), None)(paras[0]) is None

    def test_heuristic_formula_and_title(self):
        math_lines = VisualLineBuilder().build(
            [mk_glyph("f", x, 100, 14, "CMMI10") for x in (0, 12, 24)]
            + [mk_glyph("∫", 36, 100, 16, "CMSY10")], page_id=0)
        ...
        title_lines = VisualLineBuilder().build(
            [mk_glyph(c, x, 100, 24, "Helv") for x, c in
             [(0, "T"), (20, "i"), (40, "t")]], page_id=0)
        ...
        plain_lines = VisualLineBuilder().build(
            [mk_glyph(c, x, 100, 12) for x, c in [(0, "A"), (12, "B")]],
            page_id=0)
...
class TestDualPatchApply:
    ...
    def test_render_hybrid(self):
        ...
        out = OverlayRenderer(dpi=150).render_hybrid(
            page, [OverlaySegment(text="译文文本", bbox=(40, 40, 200, 56),
                                  font_size=12)], orig)
        assert len(out) > 1000
