...
def _client(monkeypatch, service=None):
    if service is not None:
        monkeypatch.setattr(
            type(service), "submit_task",
            lambda self, request: (getattr(self, "_fake_id", None)
...
class TestSubmit:
    def test_submit_maps_form_fields(self, monkeypatch, fresh_service):
        ...
        client = TestClient(create_api_app(service=fresh_service), base_url="http://127.0.0.1:11009")
        resp = client.post(
            "/api/tasks",
            data={
    ...
    def test_submit_rejects_bad_extra_config(self, monkeypatch, fresh_service):
        monkeypatch.setattr(
            RuntimeService, "submit_task", lambda self, r: "task_x"
        )
        client = TestClient(create_api_app(service=fresh_service), base_url="http://127.0.0.1:11009")
...
class TestTaskLifecycle:
    ...
    def failed_task_client(self, fresh_service):
        ...
        return TestClient(create_api_app(service=fresh_service), base_url="http://127.0.0.1:11009"), fresh_service
...
class TestSseStream:
    ...
    def test_last_event_id_replay(self, fresh_service):
        ...
        fresh_service._store.add_event(
            tid,
            TaskProgressEvent(task_id=tid, stage="parsing", progress=10.0,
                              message="first", timestamp=_time.time()),
        )
        fresh_service._store.add_event(
            tid,
            TaskProgressEvent(task_id=tid, stage="translating", progress=50.0,
                              message="second", timestamp=_time.time()),
        )
        frames = []
        with client.stream(
