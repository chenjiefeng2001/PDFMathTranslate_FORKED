...
def _entry(block_id, box, commands, text="t", font_size=11.0, kind="flow"):
    ...
    return {
        "block_id": block_id, "page": 1, "kind": kind,
        "src_box": list(box), "dst_box": list(box),
        "text": text, "translated": text, "font_size": font_size,
...
class TestDrawnExtentFloor(unittest.TestCase):
    def test_receiver_glyph_top_binds_the_floor(self):
        ...
        r = _blk(*_R)
        self.assertEqual(
            _ordered_cascade_plan([u, l, r], 792.0,
                                  excess_by_key={(1, 2): 6.8}),
            {1: 3.2})
    ...
    def test_preserved_receiver_gets_glyph_top_floor_too(self):
        ...
        self.assertEqual(
            _ordered_cascade_plan([u, l, f], 792.0,
                                  excess_by_key={(1, 2): 3.0}),
            {1: 7.0})
    ...
    def test_cascade_receivers_stay_at_final_glyph_top(self):
        ...
        plan = _ordered_cascade_plan(
            [b0, b1, b2], 792.0, excess_by_key={(1, 2): 4.0})
...
class TestDrawExtentMapBuilder(unittest.TestCase):
    def test_command_excess_enters_and_command_less_zero(self):
        ...
        plan = [
            ...
            _entry("p1_2", (60.0, 540.0, 260.0, 590.0),
                   commands=[{"y": 588.0, "x": 62.0, "text": "t",
                              "font_size": 11.0}]),
        ]
    ...
    def test_command_bottom_spill_is_never_double_counted(self):
        ...
        cmd = _entry("p1_0", (60.0, 540.0, 260.0, 590.0),
                     commands=[{"y": 530.0, "x": 62.0, "text": "t",
                                "font_size": 11.0}])
        plan = [cmd]
        excess_by_key, spill_by_key = _recovery_draw_extent_by_key(plan)
    ...
    def test_command_less_wrapped_spill_enters(self):
        ...
        text = " ".join(["a", "b", "c", "d", "e", "f", "g", "h", "i",
                         "j", "k", "l", "m", "n", "o", "p", "q", "r",
                         "s", "t", "u", "v", "w", "x", "y", "z",
                         "aa", "bb"])
        slim = _entry("p1_0", (60.0, 600.0, 260.0, 680.0), commands=[],
                      text=text, font_size=20.0)
...
class TestResolveShifts7g5(unittest.TestCase):
    def test_receiver_glyph_top_applied_end_to_end(self):
        ...
        self.assertEqual(final[1].resolved_bbox,
                         (60.0, 596.8, 260.0, 676.8))
