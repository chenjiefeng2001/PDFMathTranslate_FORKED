...
def _flow(text, max_width=200.0, max_height=400.0):
    return FlowText(
        text=text, origin=(40.0, 40.0), max_width=max_width, max_height=max_height
    )
...
def test_flow_long_wraps_to_multiple_lines():
    r = lay_out(
        _flow("This is a long paragraph of flow text that must wrap"), measure=_measure
    )
...
def test_anchor_shrink_mechanism_exercised_when_enabled():
    ...
    r = lay_out(
        prim, measure=_measure, allow_shrink=True, min_font_size=3.0, font_size=_SIZE
    )
    assert r.policy is OverflowPolicy.SHRINK
