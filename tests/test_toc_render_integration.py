...
def _build_toc_plan_entry(
    ...
    cmds = renderer.render(entries, ys=[750.0 - level * 16.0], size=10.0, translate=lambda s: f"译_{s}")
    return {
...
class TestTocPdfIntegration(unittest.TestCase):
    ...
    def _render(self, toc_blocks):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            pdf_bytes, _ = render_plan_to_pdf(
                plan, page_sizes={0: [612.0, 792.0]}, output_path=out,
    ...
    def test_title_is_translated(self):
        cmds = _build_toc_plan_entry(title_x=72.0, page_x=500.0, title_only="Introduction")
        text, words = self._render([({ "text": "Introduction", "translated": "译_Introduction",
                                       "toc_commands": cmds,
                                       "src_box": [72, 750, 500, 770], "dst_box": [72, 750, 500, 770] })])
    ...
    def test_page_number_not_translated(self):
        ...
        text, words = self._render([({"text": "Introduction 42", "translated": "译_Introduction 42",
                                       "toc_commands": cmds,
                                       "src_box": [72, 750, 500, 770], "dst_box": [72, 750, 500, 770] })])
    ...
    def test_numbering_not_translated(self):
        cmds = _build_toc_plan_entry(title_x=72.0, page_x=500.0, number="2.1", title_only="Dataset")
        text, words = self._render([({"text": "2.1 Dataset", "translated": "2.1 译_Dataset",
                                       "toc_commands": cmds,
                                       "src_box": [72, 750, 500, 770], "dst_box": [72, 750, 500, 770] })])
    ...
    def test_title_start_preserved(self):
        cmds = _build_toc_plan_entry(title_x=90.0, page_x=500.0, number="1", title_only="Intro")
        text, words = self._render([({"text": "1 Intro", "translated": "1 译_Intro",
                                       "toc_commands": cmds,
                                       "src_box": [90, 750, 500, 770], "dst_box": [90, 750, 500, 770] })])
    ...
    def test_page_number_x_preserved(self):
        cmds = _build_toc_plan_entry(title_x=72.0, page_x=520.0, title_only="Intro", page_number="5")
        text, words = self._render([({"text": "Intro 5", "translated": "译_Intro 5",
                                       "toc_commands": cmds,
                                       "src_box": [72, 750, 520, 770], "dst_box": [72, 750, 520, 770] })])
    ...
    def test_nested_x_positions_preserve_hierarchy(self):
        ...
        for title_x, num, title, level in [
            ...
            c = _build_toc_plan_entry(title_x=title_x, page_x=500.0, number=num, title_only=title, level=level)
            toc_cmds.extend(c["commands"])
            toc_entries.append({"number": num, "title_only": title, "level": level, "title_x": title_x})
        block_entry = {
            "text": "1 Intro", "translated": "1 译_Intro",
            "toc_commands": {"commands": toc_cmds, "translated_calls": []},
            "toc_entries": toc_entries,
            "src_box": [72, 700, 500, 770], "dst_box": [72, 700, 500, 770],
            "font_size": 10.0,
        }
    ...
    def test_long_title_keeps_page_number_position(self):
        short_cmds = _build_toc_plan_entry(title_x=72.0, page_x=500.0, title_only="Intro", page_number="12")
        long_cmds  = _build_toc_plan_entry(title_x=72.0, page_x=500.0, title_only="A very very very long title", page_number="12")
        short_page_x = [c["x"] for c in short_cmds["commands"] if c["kind"] == "page"][0]
        long_page_x  = [c["x"] for c in long_cmds["commands"] if c["kind"] == "page"][0]
        self.assertEqual(short_page_x, long_page_x, "page number x must not change when title grows")
        self.assertAlmostEqual(short_page_x, 500.0, delta=1.0)

    # ── 9. no-leader TOC 不被强制加 leader ─────────────────────────────────
    def test_no_leader_toc_forbids_forcing_dots(self):
        cmds = _build_toc_plan_entry(title_x=72.0, page_x=500.0, title_only="Intro", page_number="7",
                                      leader_present=False, dot_leader="")
        text, words = self._render([({"text": "Intro 7", "translated": "译_Intro 7",
                                       "toc_commands": cmds,
                                       "src_box": [72, 750, 500, 770], "dst_box": [72, 750, 500, 770] })])
    ...
    def test_multiline_entry_renders_all_lines(self):
        cmds = _build_toc_plan_entry(
            title_x=72.0, page_x=500.0, title_only="First line of title",
            page_number="5",
            continuation=["continuation line"],
        )
        text, _ = self._render([({"text": "First line", "translated": "译_First line",
                                   "toc_commands": cmds,
                                   "src_box": [72, 740, 500, 770], "dst_box": [72, 740, 500, 770] })])
        self.assertIn("译_First line", text)

    # ── CJK 验收 ──────────────────────────────────────────────────────────
    def test_cjk_title_rendered_with_original_geometry(self):
        cmds = _build_toc_plan_entry(title_x=72.0, page_x=500.0, title_only="引言", page_number="3")
