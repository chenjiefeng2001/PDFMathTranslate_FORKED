...
def build_toc_adaptive_extreme_output(path, width=612.0, height=792.0):
    ...
    per = 18
    lines = [(72, 110, " ".join(words[:per]), "body", 5.0), (500.0, 110, "12", "body", 5.0)]
    for k in range(1, (len(words) + per - 1) // per):
        lines.append((72, 130 + (k - 1) * 18, " ".join(words[k * per:(k + 1) * per]), "body", 5.0))
    add_page(doc, [(72, 80, "Contents", "bold", 14)] + lines, width=width, height=height)
    write(doc, path)
    return path
