...
def test_flow_width_wrap():
    ...
    r = adaptive_layout(
        _flow(text, w=80.0, h=400.0), measure=_measure, font_size=10.0,
        avail_width=80.0, avail_height=400.0, budget=_flow_budget(),
    )
...
def test_flow_height_shrink_reduces_font_with_budget():
    ...
    b = LayoutBudget(allow_wrap=True, allow_shrink=True, allow_clip=True,
                     min_font_size=8.0, max_font_reduction=0.2)
    r = adaptive_layout(
        _flow(text, w=60.0, h=20.0), measure=_measure, font_size=10.0,
        avail_width=60.0, avail_height=20.0, budget=b,
    )
...
def test_shrink_at_min_then_explicit_overflow():
    ...
    b = LayoutBudget(allow_wrap=True, allow_shrink=True, allow_clip=False,
                     min_font_size=8.0)
    r = adaptive_layout(
        _flow(text, w=30.0, h=400.0), measure=_measure, font_size=10.0,
        avail_width=30.0, avail_height=400.0, budget=b,
    )
...
def test_unbreakable_token_no_loop():
    ...
    b = LayoutBudget(allow_wrap=True, allow_shrink=True, allow_clip=True,
                     min_font_size=5.0)
    r = adaptive_layout(
        _flow(url, w=40.0, h=400.0), measure=_measure, font_size=10.0,
        avail_width=40.0, avail_height=400.0, budget=b,
    )
...
def test_recovery_is_finite_never_while_loop():
    ...
    r = adaptive_layout(
        ...
        avail_width=10.0, avail_height=10.0, budget=b,
    )
    assert len(r.recovery_steps) <= 3
    assert r.overflow is True
...
def test_clip_reports_overflow():
    b = LayoutBudget(allow_wrap=True, allow_shrink=True, allow_clip=True,
                     min_font_size=6.0, max_font_reduction=0.9)
    r = adaptive_layout(
        _flow("A very long sentence " * 20, w=15.0, h=8.0), measure=_measure,
        font_size=10.0, avail_width=15.0, avail_height=8.0, budget=b,
    )
...
def test_recovery_json_attached():
    ...
    b = LayoutBudget(allow_wrap=True, allow_shrink=True, allow_clip=False,
                     min_font_size=8.0)
    r = adaptive_layout(
        _flow(text, w=120.0, h=30.0), measure=_measure, font_size=10.0,
        avail_width=120.0, avail_height=30.0, budget=b,
    )
    d = r.to_dict()
    json.dumps(d)  # JSON-safe
    assert "recovery" in d
    rec = d["recovery"]
    assert set(rec) >= {"reason", "decision", "steps", "original_font_size",
                        "final_font_size"}
...
def test_recovery_json_absent_when_no_recovery():
    r = adaptive_layout(
        _flow("Fine"), measure=_measure, font_size=10.0,
        avail_width=200.0, avail_height=400.0, budget=_flow_budget(),
    )
