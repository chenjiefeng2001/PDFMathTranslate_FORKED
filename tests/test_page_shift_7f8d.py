...
def _list_with_continuation(block_id, page, x0, y0, x1, y1):
    ...
    commands = [
        ...
        {"kind": "text", "text": "content", "x": 76.0, "y": 700.0, "width": 50.0},
        {"kind": "text", "text": "cont", "x": 76.0, "y": 688.0, "width": 40.0},
    ]
    items = [
        {
            "marker": "1.",
...
def _toc(block_id, page, x0, y0, x1, y1):
    ...
    entries = [{"title": "Intro", "title_x": 72.0, "page_x": 500.0, "continuation": []}]
    return _entry(
        block_id,
        page,
        "toc",
...
class TestPureGeometryShift(unittest.TestCase):
    ...
    def test_keep_preserve_next_page_never_move(self):
        ...
        for decision in (keep_decision(b),):
        ...
        d_preserve = decide_block_shift(
            detect_page_collisions(code_plan)[0], page_height=792.0
        )
        ...
        d_next = decide_block_shift(
            detect_page_collisions(near_bottom)[0], page_height=18.0
        )
...
class TestBoundedCascade(unittest.TestCase):
    ...
    def test_budget_exhaustion_is_bounded_and_recorded(self):
        ...
        final, report = resolve_page_shifts(plan, page_sizes={1: 792.0}, max_passes=1)
        self.assertEqual(report.passes, 1)
        self.assertEqual(len(report.applied), 1)
    ...
    def test_next_page_only_stops_early_without_progress(self):
        ...
        self.assertEqual(report.deferred[0].decision, PageRecoveryDecision.NEXT_PAGE)
    ...
    def test_unparseable_block_ids_never_collapse_placement_keys(self):
        ...
        self.assertEqual([p.block_index for p in placements_from_plan(plan)], [0, 1, 2])
...
class TestGoldenGates(unittest.TestCase):
    ...
    def test_case3_toc_invariants_unchanged(self):
        ...
        page_cmd = [
            c for c in new["render_payload"]["commands"] if c["kind"] == "page"
        ][0]
        self.assertEqual(page_cmd["x"], 500.0)
        title_cmd = [
            c for c in new["render_payload"]["commands"] if c["kind"] == "title"
        ][0]
    ...
    def test_case4_code_preserve_never_moves(self):
        ...
        self.assertEqual(
            report.deferred[0].decision, PageRecoveryDecision.PRESERVE_OVERFLOW
        )
...
class TestPageShiftArchitecture(unittest.TestCase):
    def test_executor_never_redecides_or_relays_out(self):
        ...
        for banned in (
            ...
            self.assertNotIn(banned, src, f"page_shift.py 不得决策/重排: {banned}")
        for banned in (
            "list_detector",
            "list_parser",
            "toc_parser",
            ...
            self.assertNotIn(banned, src, f"page_shift.py 不得引用: {banned}")
    ...
    def test_shift_direction_contract_lives_in_one_place(self):
        ...
        self.assertNotIn(
            "- float(decision.shift_y)", src.replace("shifted = shift_box_down(", "")
