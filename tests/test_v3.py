...
try:
    ...
    from pdf2zh.v3.planner import (
        ...
        PromptManager,
        ContextBuilder,
        GlossaryEntry as PlannerGlossaryEntry,
        GlossaryManager,
        ChunkStrategy,
        ChunkSplitter,
        PlannerConfig,
...
class TestParser(unittest.TestCase):
    ...
    def test_rawblock_text(self):
        b = RawBlock(
            ...
            spans=[RawSpan(text="Hello "), RawSpan(text="World")],
        )
    ...
    def test_font_size_avg(self):
        b = RawBlock(
            block_type=RawBlockType.TEXT,
            spans=[RawSpan(text="A", font_size=12), RawSpan(text="B", font_size=14)],
        )
...
class TestNormalizer(unittest.TestCase):
    ...
    def test_normalize(self):
        ...
        result = n.normalize(
            [RawBlock(RawBlockType.TEXT, spans=[RawSpan(text="Hello World")])]
        )
    ...
    def test_sanitize_text_keeps_valid_unicode(self):
        self.assertEqual(
            Normalizer.sanitize_text("Hello – 中文 «ταБългерия»"),
            "Hello – 中文 «ταБългерия»",
        )
...
class TestGraph(unittest.TestCase):
    def test_document_node(self):
        n = DocumentNode(
            "n1", NodeType.PARAGRAPH, (0, 0, 100, 20), text="Hello", page_num=0
        )
...
class TestPlanner(unittest.TestCase):
    ...
    def test_context_window(self):
        cw = ContextWindow(
            preceding_texts=["A"], following_texts=["C"], doc_title="Doc"
        )
...
class TestDocumentMemory(unittest.TestCase):
    ...
    def test_entity(self):
        self.m.remember_entity(
            "LLM", "Large Language Model", definition="Model", translation="大语言模型"
        )
    ...
    def test_abbreviation(self):
        ...
        self.assertEqual(
            self.m.expand_abbreviation("nmt"), "Neural Machine Translation"
        )
...
class TestVisualTree(unittest.TestCase):
    ...
    def test_bbox_contains(self):
        self.assertTrue(
            BoundingBox(10, 20, 100, 100).contains(BoundingBox(20, 30, 30, 20))
        )
        self.assertFalse(
            BoundingBox(20, 30, 30, 20).contains(BoundingBox(10, 20, 100, 100))
        )

    def test_bbox_overlaps(self):
        self.assertTrue(BoundingBox(0, 0, 50, 50).overlaps(BoundingBox(25, 25, 50, 50)))
        self.assertFalse(
            BoundingBox(0, 0, 50, 50).overlaps(BoundingBox(100, 100, 50, 50))
        )
...
class TestEvaluator(unittest.TestCase):
    ...
    def test_translation_eval(self):
        ...
        o.add_node(
            DocumentNode(
                "a", NodeType.PARAGRAPH, (0, 0, 100, 20), text="Original", page_num=0
            )
        )
        t.add_node(
            DocumentNode(
                "a", NodeType.PARAGRAPH, (0, 0, 100, 20), text="Translated", page_num=0
            )
        )
    ...
    def test_semantic_eval(self):
        ...
        o.add_node(
            DocumentNode(
                "h1", NodeType.HEADING, (0, 0, 500, 25), text="Heading", page_num=0
            )
        )
        o.add_node(
            DocumentNode(
                "p1", NodeType.PARAGRAPH, (0, 25, 500, 100), text="Para", page_num=0
            )
        )
        t.add_node(
            DocumentNode(
                "h1", NodeType.HEADING, (0, 0, 500, 25), text="Heading_tr", page_num=0
            )
        )
        t.add_node(
            DocumentNode(
                "p1", NodeType.PARAGRAPH, (0, 25, 500, 100), text="Para_tr", page_num=0
            )
        )
...
class TestScheduler(unittest.TestCase):
    ...
    def test_topological_sort(self):
        tg = TaskGraph()
        a = Task("a", "A", priority=20)
        b = Task("b", "B", priority=10)
        c = Task("c", "C", priority=5)
        c.depends_on("a")
        c.depends_on("b")
        tg.add_task(a)
        tg.add_task(b)
        ...
        self.assertLess(
            next(i for i, t in enumerate(order) if t.id == "a"),
            next(i for i, t in enumerate(order) if t.id == "c"),
        )
    ...
    def test_executor_selective(self):
        ...
        a = Task("a", "A", handler=h)
        b = Task("b", "B", handler=h)
        b.depends_on("a")
        tg.add_task(a)
        tg.add_task(b)
