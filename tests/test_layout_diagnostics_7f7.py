...
def _flow_entry(text, translated, x0=60.0, y0=100.0, x1=160.0, y1=130.0,
                font_size=11.0, block_id="p1_3", page=1):
    ...
    line.spans.append(SpanModel(size=font_size, text=text, x0=x0, y0=y0,
                                x1=x1, y1=y1))
    block = BlockModel(text=text, kind="paragraph", x0=x0, y0=y0, x1=x1,
                       y1=y1, lines=[line],
                       metadata={"translated": translated})
    payload = build_block_flow_payload(block)
    return {
        "block_id": block_id, "page": page, "kind": "flow",
...
def _list_entry():
    ...
    plan = build_page_list_plan(
        ["1. Alpha", "   a. Beta"],
        geom=[{"x0": 60.0, "x1": 200.0, "size": 11.0, "y0": 700.0},
              {"x0": 76.0, "x1": 200.0, "size": 11.0, "y0": 680.0}],
        translate=lambda s: "译_" + s,
    )
    return {
        "block_id": "p1_4", "page": 1, "kind": "list",
...
def _toc_entry():
    ...
    entries = [
        {"title": "Introduction", "title_only": "Introduction", "number": "",
         "level": 0, "page_number": "42", "title_x": 72.0, "page_x": 500.0,
         "indent": 72.0, "dot_leader": "........", "leader_present": True,
         "continuation": [], "bbox": [72.0, 0.0, 500.0, 16.0]},
        {"title": "Method", "title_only": "Method", "number": "",
         "level": 0, "page_number": "12", "title_x": 72.0, "page_x": 500.0,
         "indent": 72.0, "dot_leader": "........", "leader_present": True,
         "continuation": [], "bbox": [72.0, 0.0, 500.0, 16.0]},
    ]
    cmds = TocRenderer(measure_width=None).render(
        entries, ys=[750.0, 730.0], size=10.0,
        translate=lambda s: "译_" + s,
    )
    return {
        "block_id": "p1_5", "page": 1, "kind": "toc",
...
def _code_entry():
    return {
        "block_id": "p1_6", "page": 1, "kind": "code",
        "text": "def f(): return 42", "translated": "def f(): return 42",
...
def _mixed_plan():
    ...
    return [
        _flow_entry("This is a translated paragraph that wraps over lines",
                    "ALPHAFLOW " * 12, x0=60.0, x1=140.0,
                    block_id="p1_0", page=1),
        _flow_entry("short", "short", x0=60.0, x1=300.0,
                    block_id="p1_1", page=1),
        _list_entry(),
        _toc_entry(),
        _code_entry(),
    ]
...
class TestRecoveryTrace(unittest.TestCase):
    def test_trace_recorded_per_stage_for_full_ladder(self):
        r = adaptive_layout(
            FlowText(text="X" * 200, origin=(0.0, 0.0), max_width=20.0,
                     max_height=10.0),
        ...
        self.assertEqual([t["decision"] for t in r.recovery_trace],
                         ["SHRINK", "CLIP"])
        for t in r.recovery_trace:
            self.assertEqual(set(t), {"decision", "overflow", "line_count",
                                      "font_size"})
    ...
    def test_trace_full_ladder_for_breakable_token(self):
        ...
        r = adaptive_layout(
            FlowText(text="alpha beta gamma " * 40, origin=(0.0, 0.0),
                     max_width=30.0, max_height=10.0),
            measure=_measure, avail_width=30.0, avail_height=10.0,
            font_size=11.0,
        )
        self.assertTrue(r.overflow)
        self.assertEqual(r.recovery["decision"], "clip")
    ...
    def test_trace_wrap_only_when_shrink_disabled(self):
        ...
        budget = LayoutBudget(allow_wrap=True, allow_shrink=False,
                              allow_clip=False)
        r = adaptive_layout(
            FlowText(text="X" * 200, origin=(0.0, 0.0), max_width=20.0,
                     max_height=10.0),
            measure=_measure, avail_width=20.0, avail_height=10.0,
            font_size=11.0, budget=budget,
        )
        self.assertTrue(r.overflow)
    ...
    def test_trace_does_not_change_recovery_dict_or_to_dict(self):
        ...
        self.assertEqual(
            set(r.recovery),
            {"reason", "decision", "steps", "original_font_size",
             "final_font_size"},
        )
        d = r.to_dict()
...
class TestDiagnosticsSchema(unittest.TestCase):
    def test_flow_diagnostic_carries_settled_result(self):
        entry = _flow_entry("ALPHAFLOW " * 12, "ALPHAFLOW " * 12,
                            x0=60.0, x1=120.0)
    ...
    def test_overflowing_flow_diagnostic_exposes_recovery_and_trace(self):
        entry = _flow_entry("alpha beta gamma " * 40, "alpha beta gamma " * 40,
                            x0=60.0, x1=100.0,
                            font_size=12.0, block_id="p2_0", page=2)
        diag = diagnostic_from_plan_entry(entry)
        ...
        self.assertEqual([t["decision"] for t in diag.trace],
                         ["WRAP", "SHRINK", "CLIP"])
        self.assertEqual(diag.primitive_kind, "flow")
    ...
    def test_summary_counts(self):
        ...
        plan.append(_flow_entry("A" * 500, "A" * 500, x0=60.0, x1=100.0,
                                font_size=12.0, block_id="p2_9", page=2))
...
class TestStableFields(unittest.TestCase):
    def test_stable_fields_are_architectural_only(self):
        ...
        for noisy in ("source_text", "translated_text", "bbox", "trace",
                      "font_size"):
            self.assertNotIn(noisy, s)

    def test_stable_fields_ladder_for_overflow(self):
        entry = _flow_entry("alpha beta gamma " * 40, "alpha beta gamma " * 40,
                            x0=60.0, x1=100.0, font_size=12.0)
...
class TestDebugLayoutCli(unittest.TestCase):
    def test_dump_layout_debug_writes_json(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(data["summary"]["blocks"],
...
class TestDiagnosticsArchitecture(unittest.TestCase):
    def test_diagnostics_module_is_pure(self):
        ...
        for banned in ("detector", "parser", "translator", "renderer",
                       "wrap_lines", "shrink_to_fit", "clip_text",
                       "lay_out(", "adaptive_layout(", "level *", "index *"):
            self.assertNotIn(banned, code,
                             f"diagnostics.py must not reference {banned!r}")
    ...
    def test_collector_is_read_only_view_of_settled_payload(self):
        ...
        entry = _flow_entry("A" * 500, "A" * 500, x0=60.0, x1=100.0,
                            font_size=12.0)
        diag = diagnostic_from_plan_entry(entry)
        self.assertTrue(diag.overflow)
