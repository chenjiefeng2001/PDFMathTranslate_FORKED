...
| 组件占位未连接 | 上传/配置/进度/诊断/预览均已接入 `t_inputs`（22 输入）与 `sync_outputs`（16 输出）；`gr.PDF` 启动崩溃改为 `gr.HTML + iframe` | ✅ 已修复 |
| 预览 `{"detail":"Not Found"}` | Gradio 5 `launch()` 重建 FastAPI app，`launch()` 前注册的 `/pdf-preview/` 被丢弃 | ✅ 已修复（launch 后注册） |
| 刷新导致下载失败/选项重置 | `SESSION_JS` 用 `localStorage` 持久化任务/预览/结果与配置项 | 🔶 已实现，待浏览器回归 |
| 无法选择 dual/mono | `result_files_dropdown` + `_on_select` 更新 `selected_file`；预览固定为 dual | 🔶 下载可选已接，预览切换待增强 |
