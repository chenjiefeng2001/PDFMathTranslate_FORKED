"""7J-3B — E2E translation of the AI book with CURRENT babeldoc 0.6.4.

Uses the same stub-CJK translator as 7I-7C so the run is offline.  Purpose:
inspect the FRESH output's footer ToUnicode key space and text layer, to
decide whether the Case A defect reproduces on the current stack.
"""
...
async def run_book(out_dir: Path) -> None:
    ...
    cfg = YadtConfig(
        ...
        # The AI book has full-page background images; without this BabelDOC's
        # scanned-file heuristic aborts the run before the font pipeline.
